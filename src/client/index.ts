...
export function apply(ctx: ClientContext): void {
  ...
  ctx.slots.inject('shell.overlay', () => ctx.slots.register(
    ...
    () => {
      ...
      return React.createElement('div', { className: 'dsx-stats-rail', style: { position: 'fixed', top: 'var(--dsx-rail-top,0px)', right: 'var(--dsh-sidebar-width, 0px)', bottom: 0, width: `${railW}px`, overflowY: 'auto', boxSizing: 'border-box', padding: `${pad}px`, background: 'var(--dsw-alias-bg-base)', pointerEvents: 'auto', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: `${pad}px` } },
