...
export function ccWhoamiRender(stats: WidgetStats): WidgetRenderOut | null {
  ...
  if (!user) return { title: t('widget.cc-whoami.name'), value: '—', legend: t('cc.notConfigured') }
...
export function ccUsageRender(stats: WidgetStats): WidgetRenderOut | null {
  const c = cc(stats)
  const u = c?.usage
  if (!u) return { title: t('widget.cc-usage.name'), value: '—', legend: t('cc.notConfigured') }
...
export function ccCreditsRender(stats: WidgetStats): WidgetRenderOut | null {
  ...
  if (!credits && !windows) return { title: t('widget.cc-credits.name'), value: '—', legend: t('cc.notConfigured') }
...
export function ccWindowsRender(stats: WidgetStats): WidgetRenderOut | null {
  const c = cc(stats)
  const windows = c?.credits?.windowLimits
  if (!windows) return { title: t('widget.cc-windows.name'), value: '—', legend: t('cc.notConfigured') }
  const fhPct = winPct(windows.fiveHour)
  const wkPct = winPct(windows.weekly)
  if (fhPct === null && wkPct === null) return { title: t('widget.cc-windows.name'), value: '—', legend: t('cc.notConfigured') }
...
export function ccSubscriptionRender(stats: WidgetStats): WidgetRenderOut | null {
  ...
  if (!sub && !plan) return { title: t('widget.cc-subscription.name'), value: '—', legend: t('cc.notConfigured') }
