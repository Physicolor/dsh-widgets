...
const tokensPerCredit = periodTokens / consumedCredits
const recommend = (remainingCredits * tokensPerCredit) / daysLeft
...
check('no percentage -> null (数据不足)', planQuota({ ...base, usedPct: undefined }, now) === null)
...
check('too little elapsed (< 6h) -> null', planQuota({ ...base, periodStart: new Date(now.getTime() - 3 * 3_600_000).toISOString() }, now) === null)

console.log(`\ncard: 额度管理 + 右上 ${Math.round(plan.projectedPct)}% | 账期 ${Number(plan.periodEndIso.slice(5, 7))}-${Number(plan.periodEndIso.slice(8, 10))} | 今日用量 ${fmtQuota(plan.todayTokens)} / 今日推荐 ${fmtQuota(plan.todayRecommend)}`)
...
writeFileSync(`${HERE}probe-quota-manage-result.json`, `${JSON.stringify({
  ...
  derived: { usedPct, paceTokensPerDay: pace, projectedPct, todayTokens: plan.todayTokens, todayRecommend: recommend },
  display: {
    value: `${Math.round(plan.projectedPct)}%`,
    reset: `${Number(plan.periodEndIso.slice(5, 7))}-${Number(plan.periodEndIso.slice(8, 10))}`,
