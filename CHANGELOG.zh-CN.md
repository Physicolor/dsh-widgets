...
### 新增 — host 读整池，不再只读主 Key

- **`/api/commandcode-usage` 聚合每一个已配置的池成员。** `COMMANDCODE_API_KEY` 与 provider「账户轮换」卡片写下的 `_2 … _4` 全部走同一个 credentials seam 解析，每个成员各自请求四个官方端点（`whoami` / `usage/summary` / `billing/credits` / `billing/subscriptions`），仍是各自独立 fetch、独立超时——某个端点失败只让那一片为 `null`，某把 Key 失败只让那个成员 `data: null`，其余照常渲染。
- **成员用它自己 `/alpha/whoami` 读到的账户名做标签**（`name` → `userName`），这正是图例行显示的内容（`Physicolor`、`Sparxie`，而不是 `Key 2`）。whoami 没答上来的成员退回 `Key N`；账户名重复的（同一账号两把 Key）追加该 Key 的尾 4 位，因此循环里永远不会出现两个无法区分、会切错成员的条目。
- **载荷保持向后兼容。** 顶层四片仍是第一个成员自己的四片——这正是池化之前所有消费方读到的形状——池作为 `keys[]` 并行下发。因此单 Key 安装读到的与改动前逐字节一致（验证：探针把 `keys` 剥掉后渲染出完全相同的卡片）。

### 新增 — 点卡片切换账号

- **整卡点击循环池视图。** 八张 Command Code 卡片，加上读同一账户的「额度管理」预测卡，都带循环描述：`AllUser → <账户名> → AllUser`，完整链条进悬停提示。选择按卡片实例持久化在 `cardConfigs.ccView`——刻意与 OpenCode 池的 `poolView` 分开，两族永不共享视图，Command Code 的点击也不会触发 multikey 池的 `prefer` 调用（验证：每张卡 `cycle.store === 'ccView'`）。
- **`AllUser` 是整池合计，逐字段相加。** 窗口百分比 = `Σused / Σcap`——百分比均值会把「一个用尽的池 + 一个没动的池」说成 50%，而这两个池合起来其实用掉了额度的一半（实测：5h 0.48% → 0.5%，周 7.09% → 7.1%）。请求数、token、消费相加；`successRate` 由合计后的计数重算，因为零请求的成员上游会报 0，不能把整池成功率拖下来。账户卡在合计视图里显示通用标签 `AllUser`，底下列出各账户名——`AllUser` 是刻意的中英同形表达。
- **图例行带出当前视图**，与卡片角色词并列（`账户 · AllUser`、`窗口 · Physicolor`、`5h 窗口 · Physicolor`），所以任何一个数字属于哪个池都不会含糊——单窗口卡片尤其如此，角色词是唯一说明它是哪个窗口的东西。
- **只配置一把 Key → 完全没有切换器。** 卡片与改动前渲染一致（图例行只剩角色词、账户名回到正文、无 cycle），从未配置第二个账号的安装看到的仍是原来的样子。
- **验证**：`docs/probe-cc-pool.mjs` 用假 ctx 加载真实 host 产物、对真实账号调用真实 `/api/commandcode-usage` handler，再把本仓库的 `cc-view.ts` 现编后用真渲染工厂在三种状态下渲染——AllUser、每个账号、以及剥掉 `keys` 的单池——断言标签、各项相加（计数、消费、余额、两个窗口、月窗口）、图例行、持久化字段、过期视图回退到 AllUser、以及单池卡片不变。**ALL PASS，68 条断言**（`docs/probe-cc-pool-result.json`）。

### 变更 — 月窗口按成员各自套餐相加

- **整池的月 = 各成员自身月窗口之和。** 套餐 → 额度表是按 plan id 索引的，用整池余额去对单一套餐的额度，会把实际用了 14.5% 的整池算成 **0%**。`monthlyWindow()` 现在把每个成员自己的月窗口相加，**每个成员按自己的套餐**计量（两个 GOAT 池 = 2 × $70），这也正是 roadmap 里记的「不依赖上游补字段」的读法；套餐未知的成员保留自己的余额回退，而不会污染整池。显示的 reset 取最早的成员账期结束时间——下一次续订。
- **过期视图绝不会在不再匹配的标签下显示另一个账号的数字**：持久化的 `ccView` 解析不出来时（账号改名、Key 被删）回退到 `AllUser`。
- **README 的月窗口说明是过期的**——它仍在描述已废弃的 `used / (used + remaining)` 守恒式，并把多 Key 支持列为 roadmap 项。两处现在都与代码一致；仍未做的限制也如实写明：池的发现遵循 `…_API_KEY` / `_2 … _4` 命名约定，因为 credentials 服务只暴露 `resolve(ref)`，没有「列出所有 ref」的口子可以枚举。
- **单 Key 探针一直在断言同一条过期公式**（自套餐额度修复后就一直失败）；`docs/probe-cc-render.cjs` 现在断言基于额度的数字（`28.9%`，而不是 `34.8%`），并断言单池载荷不带切换器。

### 修复 — 额度管理 不再打印「数据不足」，头部改成「上下文已用」那套版式

- **卡片再也没有「数据不足」这个状态**，缺什么就填什么：还没拿到载荷 → 大数字 `-%`，能读到账期就照常显示账期，两个数字走 `—`；账期太年轻推不出预测 → 显示真实的已用百分比；从未用过的账号 → 全是 0。旧分支把这条消息塞进标题行的右侧槽位，也正是它把蓝色标题挤成「额…」的原因——现在任何状态下标题都不会被截断。
- **账期太年轻是一个标志，不是 null。** `planQuota` 现在区分「输入缺失」（仍是 `null`）与「还没有速率可外推」（`projectable: false`），于是 6 小时下限不再把一张「已用百分比、今日用量、今日预算都真实存在」的卡整张抹掉。对第二个池成员实测（当天早上开通，已过 0.98 h）：卡片显示 `0.0%` + `账期 10-20` + `今日用量 0` + `今日推荐 251M`，而旧版是「数据不足」、没有账期也没有数字。
- **没用过的成员，那些 0 是它自己的。** 每日日志是整台机器的，只有在本账期真的花过钱的账号才会被记账（自己汇率不可得时 `todayTokens = 0`）；它的**预算**照旧有答案——剩余额度 ÷ 剩余天数，按整池已实现的「本地 token/credit」汇率折算——因为余额和账期都是真的。速率侧绝不借用该汇率：把机器的用量投影到一个没出过力的账号上等于造数。
- **灰块在数字的行盒里垂直居中，两个 token 数字回到底部。** 按基线对齐（上下文已用那种单行灰字的做法）会让 `账期 10-10` 落在数字底边下方约 8px、离下面那行数字只剩约 12px；而把正文顶到头部下方，又会让数字行紧贴 账期、卡底空出约 55px。现在 `headAfter.smallLines` 走居中，并新增 `bodyAnchor: 'bottom'` 让额度管理恢复其它卡片那种「底部落座」姿态（foot 用 `marginTop: auto`）——实测：数字底部到卡底 **14px**（内边距 13px），头部底 132.5 vs 数字顶 185.2。标题行与大数字行之间的标准间距：150px 卡上 **4px**（沿用 4/8/12/16 节奏），不再是灰字需要的 2px。
- **`headAfter.big` 现在也认 `valueTone`/`valuePulse`**（`dsx-stats-card-value` + 呼吸类名），超预算的红色闪烁跟着数字搬出标题行后依然生效——在真实 React 树里断言过，而不只是渲染对象。
- **GPU 利用率 用同一套头部**：蓝色标题在顶，`0%` 在下一行、`53°C · 0.0 GB` 在它右侧，弹性折线依旧精确填满卡盒（`card 160 = slot 160`）。它「历史点还不够画线」的状态现在显示当前利用率与事实，而不是一句等待提示。
- **两个数字重新是同一个故事，而且是精确的。** 预算之前除以向上取整的天数，而预测用精确的剩余跨度，于是「速率落在预算的 3–4% 带内」时会出现速率超预算、预测却仍显示 98%（2026-09-20 实测：速率 258.9M vs 预算 253.4M，预测 98.5%）。现在两者都用精确跨度，`预测 > 100% ⇔ 速率 > 预算` 从近似变成恒等式。
- **真实 GUI 复验**：`docs/probe-card-head.cjs`（45 条断言，Playwright + 自签浏览器会话 cookie）从**静息 deck** 读取卡片（放大浮层里是同一批卡片的旧副本），每个池视图都由**真实点击**到达，并逐视图断言头部版式、4px 间距、灰字在右的几何、灰块是否落在数字行盒内、两个数字是否落座卡底、有无「数据不足」与两个数字本身。2026-09-20 实测：AllUser `49% | AllUser / 账期 10-10 | 77.2M / 612M`、Physicolor `99% | Physicolor / 账期 10-10 | 81.2M / 264M`、Sparxie `0.0% | Sparxie / 账期 10-20 | 0 / 251M`；截图见 `docs/probe-card-head-*.png`。数学侧由 `docs/probe-quota-manage.mjs`（逐成员明细 + 新的降级规则）覆盖，真实 React 树由 `docs/verify-quota-manage-render.cjs` 覆盖。
- **顺手修掉 SSR 探针那条过期前提**：它自己的 `previewStats` 已经漂到约 103% 的月份，所以「预览默认是平静态」一直在失败——用 HEAD 的 `quota-math` 对同一组输入算也是 102.8%（已实测）。预览现在落在约 70%，超额状态留给那个本就为它存在的 sim 开关。

### 修复 — 额度管理 头部只留账期一行，且与大数字底部对齐

- **头部不再显示池视图名**（`AllUser` / 账户名）。两行灰字堆在一起显得杂乱，现在只打印 `账期 10-10` 一行，并落到头部行的**底部**，让它的底边与大数字对齐。这是契约里的新字段（`headAfter.smallAlign: 'bottom'`）；默认仍是基线对齐，「上下文已用」卡片那行灰字保持不变。点击卡片照旧循环池视图、数字跟着变——只是不再有标签。
- **实测**（`docs/probe-card-head.cjs`，53 条断言 **ALL PASS**）：`68% | 账期 10-10 | 473M / 616M`（AllUser）、`132% | 账期 10-10 | 473M / 257M`（Physicolor）、`51% | 账期 10-20 | 473M / 1.0B`（Sparxie）；三个视图里账期行的底边都精确落在大数字底边（Δ=0px），卡片上任何位置都不再出现视图名，两个 token 数字仍距卡底 14px。`docs/verify-quota-manage-render.cjs` 在渲染出的 React 树里断言同一件事（`align-items:flex-end`、没有堆叠块）。
- **探针的状态恢复移进 `finally`**（host 重读也加了重试）：曾有一次 ECONNRESET 把房间状态留在了 walk 最后点开的那个视图上。

### 修复 — 额度管理 只计量 Command Code 套餐自己的用量，不再把整台机器的用量算进去

- **卡片的 token 侧改为只折 `commandcode` 这一条路由。** 大数字、今日推荐与 credits 描述的都是「一个套餐」，而 今日用量、近期速率与 credit→token 汇率却取自全机器每日日志——于是在同一时间段跑着的 OpenCode Go 池被算到了 Command Code 的额度上（2026-09-20 实测：卡片打出 今日用量 **758M**，而这条路由自己只服务了 **474M**；全机器当日 **783M**，其中 309M 属 `opencode-go`）。新增 `stats.commandCodeDaily` 承载这份带 provider 口径的日志，卡片不再读别的。
- **`/api/widgets-usage-daily?provider=<route>`** 让 dsh-usage-center 只折这一条路由。mode 必须是 `only`：它的 `all`/`merge` 会刻意保留整个窗口，等于把全机器地图原样还给一个具名路由。不带参数时路由行为完全不变，heatmap 系列卡片不受影响。
- **两张地图分开持有；旧 host 只会降级、不会说谎。** 客户端要求带口径的回答**回显** `provider=commandcode`；未重启的 host 会忽略该参数，此时卡片两个 token 数字打印 `—`，而不是悄悄退回混合口径。
- **`docs/verify-cc-scope.mjs` 独立仲裁**：它自己解码每一份原始会话日志（不用任何插件代码），按 **provider 路由**折出每日 token，再把卡片口径与 host 两张地图、真实账户载荷三方对照——2026-09-20：`commandcode` 473.7M vs 全机器 783.3M，被排除的 `opencode-go` 309.5M。`docs/verify-usage-daily-route.mjs` 离线覆盖路由的两种口径，`docs/probe-quota-manage.mjs` 与 `docs/verify-quota-manage-render.cjs` 也改为读带口径的地图。

### 修复 — 上游某一片没答上来时，池数值不再被就地换口径

- **池缺片就降级，绝不报一个混合口径。** host 路由对失败的上游调用写 `null`（一个端点挂掉不能把整族卡片清空），而客户端以前是默默吸收的：某个成员的 `billing/subscriptions` 掉线时，它会退回**早已废弃**的守恒口径（`used/(used+remaining)`），同时池的日历取了另一个成员的账期，于是额度管理打出 **20.1% + `账期 10-20` + `今日推荐 58.6M`**，而完整载荷是 **16.6% + `账期 10-10` + 645M**（就是开启这次修复的那张截图，2026-09-20）；某个成员的 `billing/credits` 掉线时，池塌缩成存活成员那一个窗口，打出**假的 117% 超额告警**。现在 `monthlyWindowOf` 不再把「缺 subscription 片」当作「冷门套餐」，`monthlyWindow` 只要有一个成员摆不进月份就返回 `null`——部分求和的读数**低于**真相，而额度卡读数偏低恰恰是用户无从察觉的那种错。
- **池日历改成「某一个成员的 (start, end) 对」**，不再是「最早的 start 配最早的 end」（本机实况是 09-10 → 10-10 与 09-20 → 10-20，两半来自两个不同成员，卡片于是量了一个谁都没有过的账期）。锚点取**仍在未来**的最早重置，所以已经滚过期的成员不会锚住整月；`账期` 行也改成打印那个时刻的**本地日**，不再直接切 ISO 字符串的 UTC 字段。
- **一片报缺失之前先重试一次。** `fetchSlice` 在「失败得快且可重试」（网络错误 / 超时 / 5xx / 429）时再问一次，且用更短的预算，最坏情况仍贴着原来 8s 的上限；4xx 是**答案**，不重试。
- **载荷降级时采集器会自己再问一次。** `ccPayloadDegraded`（有片缺失，或账期已经滚过）会在 15s 后补一次单发；于是上游瞬时故障是**一闪而过**，而不是整个会话的答案。新增 `docs/verify-quota-reask.cjs` 端到端复验：冻结一份真实完整载荷、摘掉一个 subscription 片再喂给页面，采样真实卡片：8.4s 时 `额度管理 | -% | AllUser | 账期 10-20 | 今日用量 10.1M | 今日推荐 —` → 38.3s 时 `75% | 账期 10-10 | 今日用量 480M | 今日推荐 649M`，且第二次请求是页面自己发的。
- **credit→token 速率要求日志**覆盖**整个账期。** 日志从中途开始会低估分子，而低估的速率不只是把速率算小——它把**以 credits 计的速率**算大，这正是刚打开页面时（dsh-usage-center 的日表还没到，退回浏览器本地日志，只有今天一行）把一个平静的 **75%** 月份变成 **247% 红色告警 + `今日推荐 1.5M`** 的来路。新增 `logCoversSince` 同时把住速率、速率窗口与借用的池速率（池速率改为逐成员按**各自账期**求和），覆盖不到时卡片打印已用百分比 + `—`，而不是编一个价。
- **`docs/probe-cc-pool.mjs` 增加「池完整性」一节**——两种缺片都让池月份变未知、日历是某一成员的 (start, end) 对且锚在仍有效的账期上、`账期` 标签是本地日、滚过期的账期会被标记以便补拉：**88 条断言，对真实账号 ALL PASS**。

### 变更 — 套餐卡显示等级徽标，预览点击走完整条套餐阶梯

- **套餐是徽标，不是原始 plan id。** `individual-goat` 以前是卡片的大数字，会以 `individual-goa…` 顶出卡片边缘；现在打印官方套餐表的名字（`GOAT`、`PRO`、`MAX`、`ULTRA`、`PROVIDER`、`GO`、`TEAMS PRO`），并且和 provider 一样在归一化后按**最长前缀**解析（所以 `individual-pro-v1` 胜过 `individual-pro`）。表外的 id 既不硬套等级也不原样输出：去掉厂商标识前缀、其余转大写，徽标始终是一个短词。
- **图例行变成账期**，用词与额度管理卡的 `账期 10-10` 一致（「套餐」这个角色词删掉了——徽标本身就说明了卡片是什么）；徽标落座卡片底部，只有非 active 状态或到期不续订才会在它下面补一行小字。
- **预览点击走的是「阶梯」。** `WidgetExample` 新增 `simSteps`：存在时，组件市场 / 组件配置里点击预览卡片会前进到下一个模拟状态，而不是翻转单个布尔值，于是套餐卡按 GOAT → PRO → MAX → ULTRA → PROVIDER → GO → TEAMS PRO → GOAT 循环。两个预览入口共用同一个 `nextSim()`，峰谷定价与额度管理的布尔翻转保持不变。
- **真实 GUI 复验**：`docs/probe-cc-plan-preview.cjs`（26 条断言）打开真实的市场面板、选中套餐的预览圆点，断言标题、账期行、徽标落座底部（34px vs 图例 18px）、原始 id 不出现——随后点击卡片七次，要求恰好是 `GOAT → PRO → MAX → ULTRA → PROVIDER → GO → TEAMS PRO → GOAT` 且账期行不变。`docs/probe-cc-pool.mjs`（73 条断言）用**线上载荷**覆盖同一张卡；截图见 `docs/probe-cc-plan-*.png`。

### 变更 — 用量 / 额度 并入「大字 + 底部落座」版式，组件预览变成真正的正方形

- **用量（cc-usage）：事实改走 figures 行。** token 总量仍是标题下的大字（右侧 `tokens`），但那一条灰字（`4821 请求 / 100% 成功率 / 消费 $26.5`，约 150px 塞进约 126px）**永远**把消费截断——现在三个事实是底部的一行三栏（`请求 4821 · 成功率 100% · 消费 $0.4676`），角色词删掉（数字自带单位）。
- **额度（cc-credits）：按官网的限额行重做。** 顶部 `69.16 credits`，下面三行——`5 小时 6%` / `周 2%` / `月 1%`（名字在左、百分比硬靠右），每行下面是 **24 格分段横条**（已用格按该窗口的紧急度着色，其余是同色淡底）。旧的 `kind: 'bars'` 双柱（连同虚线网格与 25px 标签）以及角色词、重置行一并删除。`WidgetChart` 新增 `quotas` 类型承载这些行。
- **预览台就是卡面。** 两个预览入口现在都把卡片钉在 unit 正方形里（`pinBox`），内容过高的卡片由自身 `overflow: hidden` 裁切，与轨道上的落座方式一致——改造前实测额度卡在组件市场渲染成 **200×250**，看起来就是个非正方形圆角矩形；现在每张预览卡都精确 200×200。
- **折线不再被切掉一半描边。** 0% 采样（GPU 空闲是日常）会映射到 y = 100，绘图盒的 `overflow: hidden` 把它的下半条描边切掉——在线上「利用率」卡实测：2px 描边下折线底 **754.0 == svg 底 754.0**，这正是用户报的「底部截断」。现在折线活动范围收到 `[PAD, 100 − PAD]`（PAD = 3 viewBox 单位），**填充**仍闭合到真正的底边；修复后实测折线底 **751.9**，盒底 754。时间标签行也核验在卡片内边距之内（底 768 vs 底线 769）。**因为用户报的是「偶现」，这条不靠信任**：`docs/probe-gpu-line-stability.cjs` 每 1.5s 采一次真实卡片、连采 45s（30 个样本——描边始终在盒内 2.1px、标签在底线内 1px、卡片高度从未离开槽位），`docs/probe-gpu-line-values.cjs` 则拦截轨道自己的 sysinfo 路由、喂一条合成的 **0 → 100 → 0 斜坡**，一次渲染就覆盖折线能取到的所有 y：实测绘图范围恰为 viewBox 的 `[3, 97]`，两端各有 2.1px 余量（描边 2px）——没有任何取值能切到它（截图 `docs/probe-gpu-line-ramp.png`）。
- **行高是按 2×2 预算算出来的，不是估的。** 额度重做的第一版三行量到 100px，而 2×2 卡在标题 + 余额之后只剩 81px，最后一根横条被卡底切掉；最终落到 9px 标签行 / 2px 间隙 / 9px 横条 / 行间 5px（合计 73px），并在真实 GUI 复验。
- **消费数字只保留三位有效数字**（`$10.1`、`$0.468`），不再是四位小数（`$0.4676`）：2×2 卡上第四位小数就是噪声，而且这个数字是和另外两个事实挤在同一行的。超过 $100 取整到元，低于十分之一美分显示 `<$0.001`，而不是把真实消费抹成 `$0`（线上实测：`$27.35` → `$27.4`、`$0.4676` → `$0.468`）。
- **虚线折线：一次「读不到」被画成了「断口」。** 用户反复看到的「底部截断」（这次是稳定复现、GPU 完全空闲）真正的根因不在描边：host 的 `nvidia-smi` 查询超时就往历史里记一个 `null` 样本，而折线渲染在 null 处**断开 polyline**——2026-09-20 线上实测：`history.gpu` 里有 **26 个 null（`history.cpu` 是 0 个）**，利用率卡渲染成 **2 条 polyline（15 + 3 个点）**，也就是用户看到的「断成一截一截」。现在 `plotSamples()` 会把上一次读数**向前带过最多 4 个连续 miss**（轮询窗口本身就是平均窗口，上一读数对利用率曲线就是最佳估计——Windows 自带曲线也是这么画的），开头的 null 直接丢掉而不是伪造，更长的连续 null 仍然断开（设备真的消失，卡片自身的「未检测到 GPU」状态也会用文字说明）。修复后实测：轨道卡与组件市场预览都是 **1 条 polyline、20 个点**。两端都有回归覆盖：`docs/probe-gpu-line-values.cjs` 喂**带 null 段**的斜坡并要求 1 条 20 点折线，`docs/probe-gpu-line-stability.cjs` 要求每个线上样本 `segments === 1`。
- **真实 GUI 复验**：两个新的探针——`docs/probe-cc-cards-preview.cjs`（23 条断言：严格 200×200 正方形、三个事实按 mock 值落座底部、三条限额行共 72 个格子且填充数跟百分比一致：6%/2%/1% 之于 24）与扩展后的 `docs/probe-card-head.cjs`（47 条断言，新增「折线描边整条留在绘图盒内」「时间标签留在卡片内边距内」）。同一对卡片在**线上载荷**下由 `docs/probe-cc-pool.mjs`（79 条）与 `docs/probe-cc-render.cjs`（24 条）覆盖；截图见 `docs/probe-cc-cards-*.png`。
- **探针学会了区分「上游缺片」与「版式缺陷」**：host 路由对每个成员的四个端点各自抓取，限流时某个成员可能缺片（跑整套时实测到）。账期断言现在把它记为上游状态而不是怪卡片，并接受 `Key N` 兜底标签作为按位置命中的视图名。

### 变更 — 悬浮放大的波动改为一条逐帧写入的弹簧

- **进入不再是 CSS 补间，而是一条连续弹簧。** 旧结构是 200ms 的 `settle` 补间接一段不做补间的 `follow`，而任何参数都救不了「移动中的目标」：指针每帧写一次新焦点就会**重定向** CSS transition，它从当前值重新起步、永远过不了缓入段的慢启动（真实 GPU 逐帧实测：指针走进卡片时速度只有 **0.02–1.1 scale/s**，而单次跳入是 **3.5**——进入是「鼠标挪多快就动多快」）。把目标冻结下来让补间跑完，曲线是好看了，但指针继续移动时几何已经陈旧，冻结一结束卡片就**瞬跳**到实时位置——正是用户报的「悬浮到组件之后一移动就错位」。
- **进度现在是一个显式因子**，每帧写入：`displayed[i] = 1 + (target[i] − 1) · p`。`target` 是指针的实时几何（永远最新，因此永远不构成「重来一次」的理由），`p` 是 `WAVE_SPRING` 在静息（`0`）与激活（`1`）之间的取值。两者都连续，乘积自然连续：进入 / 跟随 / 离开是**一条不中断的曲线**，两个方向同一公式；被打断的进入从实时的 `p` 反向，不跳变。弹簧用的是与 `dsh-usage-center` 图表补间共享的 Apple 参数化写法（`response 200ms`、`bounce 0.1`）——约 250ms 收敛、约 0.15% 过冲（默认档位下约 0.4px）。`prefers-reduced-motion` 下一帧直接到位。
- **相位机整体删除**：`animPhase` / `settle` / `follow` / `return`、`setPhaseNow` / `setPhaseAfter` / `phaseTimer`、`MORPH_MS` / `SETTLE_MS`。`leaveRail` 现在只剩 `armed = false; focus = null`——离开就是把同一条弹簧反着读，因此没有第二条需要同步的路径，也不存在「补间跑完了吗」这个会落错侧的边界。
- **进入剩下的成本是合成层成本，修法是一个常驻提示。** 放大层各槽位与「＋」按钮现在**常驻** `will-change: transform`，15 个合成层在指针到达之前就已存在。按旧写法把提示挂在 hover 上实测：自然进入的首帧是**63ms** 的层提升 + 栅格化帧；改成常驻后同一进入是 **16ms、零掉帧**（`scripts/diag-hover-isolate.cjs`，P4 vs P5）。过去在组件栏空闲时把放大层翻出来 64ms 的「栅格预热」已**删除**——它几乎从不与真实悬浮重合，而常驻提示一到位，那次卡顿就再没复现。
- **逐帧验证**：`docs/verify-rail-morph.cjs`（`HEADFUL=1` 走真实 GPU，`REDUCED=1` 走减少动画路径）对「走路进入 / 单次跳入 / 走路离开」三种手势逐帧采样并给出速度包络——被重定向的曲线或瞬跳的交接无法蒙混过关：走路进入的 `maxFrameStep` 为 **0.022**，同一几何下 CSS 版本是 **0.181**；减少动画时一帧到位。状态机与滚动分别由 `scripts/diag-rail-hover-release.cjs`（11 条断言）与 `scripts/diag-rail-scroll-perf.cjs` 做回归。

### 新增 — 设置导航项用组件自己的图标

- **设置里「组件」那一格不再显示通用齿轮。** 设置外壳按 section ID 选图标，而它只认得三个内置 ID（`models` / `agent-presets` / `plugins`），其他 section——包括本插件——一律回退成设置齿轮；section 契约里也没有图标字段（只有 `id` / `order` / `label`）。因此客户端自己给本行打标记：按**我们注册的 label** 找到那个导航按钮并写上 `data-dsx-nav="widgets"`，由 `widgets.module.css` 隐藏齿轮、用 `currentColor` 的 16px `mask` 画出 app 图标的四格 motif。
- **标记有边界、可回滚。** 只动那一行——按我们自己的 label 匹配而非位置，因此不会误伤别的插件——语言切换后重新标记（label 是本地化的），并随拥有它的 effect 一起移除：关闭面板或卸载插件都不会留下带标记的节点。观察器只在 `_navList` 插入时扫描一次，对话流逐帧的 DOM 变动不需要为它付出代价。
- **它是「恢复」而不是「新造」**：CSS 半与这段标记当初是一起写的，但只有 CSS 进了提交——打标记的 effect 随一次未提交的 `index.ts` 重写一起丢了，于是规则命中不到任何节点，齿轮又回来了。（与下面骨架剪影完全相同的失效模式；现在两半都在树里，也都有探针。）
- **实机验证**：`docs/verify-nav-glyph.cjs` 驱动真实设置面板并断言：全表**只有一行**带该属性且其 label 正是我们注册的 section 文案；该行官方齿轮 svg 计算值为 `display: none`；其 `::before` 是 16×16 的内联 SVG mask；其他导航行无一被标记；面板关闭后带标记节点数为 0（截图 `docs/verify-nav-glyph/nav-glyph.png`）。

### 修复 — 加载骨架画的是它所代表那张卡的外形

- **所有数据驱动的卡片都塌缩成了同一个通用占位**（一个数值胶囊 + 两条细线），因此加载中的组件栏只能说「这里在加载」，说不出**是哪几张卡**要来——而加载 → 加载完成的那次替换还会把卡片外形换一遍。`SkeletonBody` 一直都支持七种剪影（`WidgetRenderOut.skeletonShape` / `skeletonCount` / `skeletonRows` 本来就是公开契约的一部分），但 shell 侧的 `SKELETON_SHAPE` 表连同填它的那段接线，与上面导航图标一样随那次未提交的重写**丢失**了：装载处只写了 `skeleton: true` 与按数据源的行数，于是 19 个有数据源的组件全部落到 `skeletonShape` 的缺省值 `'text'`。消费端一直在，生产端没了。
- **表已补回，并且是按组件声明的**：`usage-rings` / `cc-windows` 画三个方块（环），`sys-rings` 两个，2×4 的 `sys-board` 四个；`usage-bars` 与 `sys-gpu-line` 画**一整块**大圆角矩形（绘图区）；`cc-usage` 是贴底的三列数字行，`quota-manage` 两列，`cc-credits` 三条堆叠额度条；其余大数字卡（`usage-rolling` / `weekly` / `monthly`、`cc-whoami`、`cc-subscription`、`cc-window-*`、`sys-cpu`、`sys-gpu`）画数值胶囊 + **一行**正文——这才是诚实占位：这些卡真实正文本来就只有一行灰字（重置日期、账期、内存行），画两行等于承诺了永远不会到的内容。
- **对全部 19 个实例验证**，而不只是本机装上的那几张：`docs/verify-skeleton-shapes.cjs` 把四条数据路由永久 hold 住、注入全部 `widget@size` 数据实例，逐张量出骨架块（数量、几何、行列顺序、圆角、溢出）并按期望剪影判定——**19/19 PASS**——随后用运行前的快照恢复用户自己的组件栏状态（`docs/lib/widgets-state.cjs`）；截图在 `docs/verify-skeleton-shapes/`。

### 新增 — 连续曲率圆角

- **卡片圆角是超椭圆，不是圆弧**：`corner-shape: squircle` 让曲率从直边连续过渡进来，这正是 iOS / Figma 式圆角矩形看起来「软」而不是「切」的原因。它是原生 CSS，因此卡片的描边与 `box-shadow` 都沿同一条轮廓（用 `clip-path` 做超椭圆会把两者都切掉）；不支持 `corner-shape` 的引擎退化为普通圆角，其他行为不变。
- **两个控件，都在 设置 → 组件 → 组件设置**：开关（默认**开**——留着是为了让用户能对比）与**圆角档位**，即半径占卡片**短边**的比例（`12 / 16 / 20 / 24 %`，默认 16%）。用百分比而非像素，因为参照物保持的是圆角与边长的比例：150px 的卡片与放大到 190px 的副本不该拿到同样的圆角；`12%` 正好还原旧的固定 16px（边长 150 时）。
- **档位到达每一个会画卡片的地方**：组件栏（静息 deck 与放大层）、组件市场 / 组件配置的预览、以及加载骨架。缺的正是这段接线——`Prefs` 已声明两个字段、设置行会写、预览会读，但组件栏自己那次 `CardBody` 调用两个参数都没传，所以设置只改得动预览；`DEFAULTS` 也漏了这两个字段（`tsc` 有报错）。
- **实机验证**：`docs/verify-corner-shape.cjs` 在 15 张卡的组件栏上断言：开关为开时每张卡都带 `dsx-squircle`，且实测圆角等于 `短边 × 档位%`（边长 160、16% 档位下为 26px）；关掉开关后 15 张全部去掉该类，再打开全部回来；选 24% 档位后所有圆角变为 38px——结束时恢复为运行前读到的值。

### 修复 — Command Code 卡片不再冻结在挂载那一刻的数字

- **「额度管理」的 今日用量 / 今日推荐 恒为 `—`**：这张卡读的是按 `commandcode` 路由折叠的日 token 地图（`stats.commandCodeDaily`），而客户端**从未请求过**那个 URL（`/api/widgets-usage-daily?provider=commandcode`）——契约、宿主路由、宿主的折叠逻辑三者都在，缺的只是发起请求那一行，于是 `daily === undefined`，两个数字一起退化成「没有这个数值」的横杠。现已接通；真实载荷下渲染为 今日用量 **363M** / 今日推荐 **726M**（头部 35%）。
- **Command Code 的月环整只消失、额度管理 头部变成 `-%`**：一次降级应答（HTTP 200，但某个上游切片为 `null`）会让 `monthlyWindow()` 按设计返回 `null`（部分求和会低报），而客户端会用这份降级载荷**覆盖**上一份完整载荷，并且只在挂载 / 回合结束时才再问一次——`ccPayloadDegraded()` 当初正是为「看到降级就立刻再问一次」导出的，却从未被调用。实测三次采样命中一次（`keys[].data.*` 全为 `null`、标签退回 `Key N`）。现在降级载荷绝不覆盖完整载荷，并会于 5 秒后补问一次；请求失败也不再清空上次数值（stale-while-error）——拿不到新值就继续显示上一次的。
- **空闲时数字永不前进**：这些窗口是**账户级**的（别的客户端用同一个池也会推动它），而旧的抓取只发生在挂载与回合结束时——这正是「用量中心在动、组件栏不动」的来源：用量中心有自己的 2 s 轮询。现在组件栏开着且装了任一 Command Code 卡片时，每 60 s 拉一次账户数据；标签页隐藏时暂停，切回标签立即刷新一次。
- **开销是量出来的**（2026-09-23 实测）：一次账户 tick = 每个池成员 4 次上游读（本机两把 Key 即 8 次、1.06–1.6 s、4.7 KB，约 480 次/小时）；而 `provider=commandcode` 的日地图是 usage-center 里没有别人使用的 memo key，冷调用要重折会话日志——**10.6 s 与 21.2 s**，热时 34 ms——因此它只在回合结束与挂载时计算，绝不进轮询。宿主路由新增 20 s TTL + single-flight 缓存：多标签、降级补问、以及一次回合结束带出的两个 scope 合并为一次上游 / 一次折叠（实测冷 1063 ms → 命中 0 ms，6 次请求只折叠 3 次）。

## v1.6.1 — 组件栏按整行滚动、滚到自己的内容就停、悬停离开必定复位

> 修复版本。v1.6.0 之后仍存在的三个组件栏缺陷全部消除：滚轮按像素滚动（可能停在半行，甚至完全不动）、能滚过最后一个组件进入空白、以及指针已经离开组件后放大波仍不结束。以下每条都在真实 GUI 上实测（1578×1000，15 个组件，2 列，行距 184px），并由 `scripts/` 中随包发布的三个验证脚本复验。

### 修复 — 滚轮按整行滚动

- **最后几行永远顶不到顶**。组件栏的可滚动范围原本是 deck 自身高度减视口高度，比最后一行的偏移还短（实测：8 行 × 行距 184，范围仅 750px），于是浏览器把末尾几个挡位 clamp 掉，滚轮还在转、deck 却不动了。现在可滚动内容由几何推导：`contentH = max(rows · pitch, stackHeight) + 组件栏视口高度`，差额由一个不可交互的尾部元素在组件栏内预留，于是每一行都能顶到顶，最后一行也能。
- **停在半行**。滚轮自身的原生滚动在事件抵达主线程**之前**就已落在合成器上（实测：处理函数运行时 `scrollTop` 已经是 120），一旦动画被打断，deck 就可能停在三分之一行的位置。现在组件栏自己跑 240ms 补间、必定落在挡位上，并有一个行守卫把任何偏离网格的静止位置吸附回 `2 + row · pitch`。
- **把补间取消掉的依赖数组**。滚轮 effect 的依赖里写了 `railElRef`——一个父组件每次渲染都会重建的普通 `{ current }` 对象——于是父组件每次重渲染都会拆掉 effect，其 cleanup 在补间进行中调用 `stopTween()`。现在 ref 是真 `React.useRef`，effect 只依赖卡片尺寸与间距。
- **line / page 模式滚轮要么不动要么跳九行**。LINE/PAGE 是离散量：一个事件就是一个挡位，与量值无关。此前 3-line 事件（48px）低于像素阈值 → 完全不动，page 事件（原始 `clientHeight`）→ 一次跳九行。
- **验证**：`scripts/verify-rail-wheel-matrix.cjs`（单事件、一次滚动拆成两个事件、触控板小刻度、快速甩动、line 模式、page 模式、向上甩动、长甩到底）与 `scripts/verify-rail-scroll.cjs`（每档恰好一行、每个静止位置都是整行、任何挡位都有一整行顶齐、最后一行可达）。

### 新增 — 滚到组件结束就停

- **阈值取代无限尾部**。只有当某一行自己的卡片仍能进入视口时，这一行才允许顶到顶：`lastRow = ceil((contentBottom + 视口高度 − 行基准) / pitch) − 1`。实测：8 行、最深卡片底 1450px、视口 936px → deck 停在第 7 个挡位（1290px），继续滚动完全不动（`1290 → 1290`）。改动前会多出四个只把空白拉上来的挡位。

### 修复 — 指针离开后放大波必定结束

- **安全网把自己取消了**。此前加过一个 window 级 `mousemove` 守卫，用于在指针不再位于卡片上时结束放大波——但它的 effect 没有依赖数组，于是波活跃期间（每帧重渲染）每次 cleanup 都会清掉待触发的释放定时器，它永远没机会执行。现在守卫只挂载一次，状态从 ref 读取。
- **「在组件上」现在指在卡片上，而不是「在组件栏里」**。组件栏的盒子是 372 × 936 且大部分是空的——内边距、卡片之间 24px 的缝隙、最后一行以下的整条空白都在其中——所以旧的盒子判据会在缝隙里、第一行上方、最后一张卡右侧继续维持放大。现在逐张卡片判定并带 7px 光晕（卡片间距 24px，放大后相邻约 12px），既覆盖穿越缝隙又不吞掉空白区域。
- **六条退出路径全部复验**：`scripts/verify-rail-interaction.cjs`——离开最左卡片、离开最右卡片、沿空列向下、向下进入 deck 下方空白带、第一行上方、快速斜穿——每条都用 `document.elementFromPoint` 核对终点，绝不会把「终点确实在卡片上」误判为缺陷。
