...
### v1.1.1
**修复**
- 🔢 Token 用量热度图跨天重复计数：`tokenUsage` 是当前会话整个事件日志的累计用量（无日期维度），而旧记账每天清晨把基线清零——今天继续昨天的会话时，会把昨天的全部累计（如 47M）误记为今天的增量。现改为锚点只在「观测到累计增长」时推进（同一会话内增长，含跨天零点），累计回落（新会话/日志重建）时重建锚点且不计入。旧的每日基线键在首次加载时自动迁移为新锚点。
