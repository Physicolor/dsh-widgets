...
### v1.1.6
**修复 — 悬浮放大改为以卡片为准；添加按钮随波放大**

- **实时（无极）模式**此前在组件区域内任意位置都会放——指针停在卡片间空隙也会触发。现改为：**只有指针真正悬浮在卡片上才触发**；在卡片间空隙过渡时保持悬浮（组件间移动不会闪灭）；**只有指针离开组件区域（如移出左/右侧）才停止**。离散模式保持同样的卡片锚定姿态。
- **底部添加按钮参与放大波**：当波峰靠近它（如悬浮其上一行的卡片）时，悬浮层镜像的添加按钮以同样的钟形曲线在静息网格位置放大——此前周围卡片都在缩放时它纹丝不动。
- 🧪 无头浏览器实测（playwright，两种模式）：rail 顶部空白区悬浮不触发（实时模式）、卡片上触发、卡片间空隙保持、移出 rail 停止；添加按钮在波峰下由 150px 放大到 166px。
