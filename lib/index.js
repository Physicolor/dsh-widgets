...
*/
function readAuthoritativeDaily(ctx) {
...
function apply(ctx) {
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (_req, res) => {
			const key = (await ctx.credentials.resolve(COMMANDCODE_KEY_ENV))?.value;
			if (key === void 0 || key === "") {
				res.writeHead(503, { "Content-Type": "application/json" });
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (req, res) => {
			if ((req?.url ?? "").includes("refresh=1") && Date.now() - lastRefreshAt >= REFRESH_THROTTLE_MS) {
