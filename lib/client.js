	...
	factory: (require) => {
		...
		function usageRingsRender(stats) {
			...
			return {
				...
				chart: {
					kind: "rings",
					rings: [
						mk("滚动", u.rolling.percent),
						mk("周", u.weekly.percent),
						mk("月", u.monthly.percent)
					]
				}
			};
		}
		...
		*  can pass meta.sim = { peak: boolean, window?: 0|1 } to force either state. */
		function peakPricingRender(_stats, meta) {
			...
			const activeLabel = simPeak !== null ? simPeak ? (PEAK_WINDOWS_BJ[sim && typeof sim.window === "number" ? sim.window : 0] ?? PEAK_WINDOWS_BJ[0]).label : void 0 : live.activeLabel;
			return {
				title: "峰谷定价",
				meter: PEAK_WINDOWS_BJ.map((w) => ({
					label: w.label,
					active: w.label === activeLabel
				})),
		...
		*  without a todos projection — shows 暂无任务 so the card never vanishes. */
		function taskRender(stats) {
			...
			return {
				title: "任务",
				value: (todos ? todos.length : 0) > 0 ? `${done} 已完成` : "暂无任务",
		...
		*  - 'coding-plan' : Token-usage heatmap + last-7-days bars. */
		const WIDGETS = [
			...
			},
			{
				...
				group: "coding-plan",
				name: "用量热度图",
				desc: "每日 Token 用量热度图（自记账）。2×2 显示近 3 个月日历，2×4 显示近半年全部用量点；大小可在市场左右切换",
				builtin: true,
				sizes: ["2x2", "2x4"],
			...
			},
			{
				...
				configSchema: [{
					...
					type: "mode",
					default: "rolling",
					options: [["rolling", "滚动(最近7天)"], ["weekly", "每周对齐"]]
				}]
			},
			{
				id: "usage-bars",
			...
			},
			{
				...
				group: "pricing",
				name: "峰谷定价",
				desc: "DeepSeek V4 峰谷定价：当前是否处于高峰时段（北京时间，工作日 09:00–12:00 与 14:00–18:00 为高峰）",
				builtin: false,
				badgeLabel: "峰谷定价",
				simToggle: "高峰/低峰",
		...
		*  variants share exactly the same source the real collector uses. */
		...
		function CardBody({ out, unit, width, onAction }) {
			...
			return react.createElement("div", {
				className: "dsx-stats-card" + (out.alert ? " dsx-peak-alert" : ""),
				style: {
					position: "relative",
					width: `${boxW}px`,
					minHeight: `${unit}px`,
		...
		function ConfigTab({ controller }) {
			...
			const previewOut = () => {
				...
				if (selWidget.id === "quote") {
					...
					if (!rawText || !rawText.trim()) stats.text = "（填写寄语内容后显示）";
				}
			...
			return react.createElement("div", { style: {
				...
				color: "var(--dsw-alias-label-tertiary)",
				marginBottom: 4
			} }, `已添加 ${installed.length}/${prefs.maxWidgets}（点击组件可预览与配置）`), react.createElement(OrderList, {
				items: installed,
				...
				const simTip = selWidget.simToggle ? react.createElement("div", {
					key: "simtip",
					style: {
						fontSize: 11,
						color: "var(--dsw-alias-label-tertiary)",
				...
				}, `点击卡片切换：${selWidget.simToggle}`) : null;
				return out ? react.createElement("div", {
					style: {
						...
						cursor: selWidget.simToggle ? "pointer" : void 0,
						userSelect: "none"
					},
					title: selWidget.simToggle ? "点击切换预览状态" : void 0,
					onClick: selWidget.simToggle ? () => toggleSim() : void 0
				}, pv, simTip) : null;
		...
		function MarketTab({ controller, usageData }) {
			...
			if (previewGroup !== null) {
				...
				return react.createElement("div", { style: {
				...
				}, "← 返回"), react.createElement("div", { style: {
					flex: 1,
					minWidth: 0,
					display: "flex",
					alignItems: "center",
				...
				} }, w ? `${w.name}${curSize === "2x4" ? " 2×4" : " 2×2"}` : ""), sizeBlocked ? react.createElement("span", { className: "dsx-size-warn" }, "1列不可用") : null), react.createElement("button", {
					type: "button",
					...
					onClick: add,
					title: sizeBlocked ? "1 列布局下不显示 2×4 组件" : void 0
				}, installed ? "已添加" : "添加")), !installed && prefs.installed.length >= prefs.maxWidgets ? react.createElement("div", { className: "dsx-limit-tip" }, `已达上限 ${prefs.maxWidgets} 个，先在组件配置中移除再添加`) : null, react.createElement("div", { style: {
					flex: 1,
					minHeight: 0,
					display: "flex",
					alignItems: "center",
					...
					style: {
						...
						transformOrigin: "center center",
						cursor: w?.simToggle ? "pointer" : void 0,
						userSelect: "none"
					},
					title: w?.simToggle ? "点击切换预览状态" : void 0,
					onClick: w?.simToggle ? toggleSim : void 0
				}, react.createElement(CardBody, {
					out,
					unit: 200,
					width: curSize === "2x4" ? 412 : void 0
				}), w && w.simToggle ? react.createElement("div", { style: {
					fontSize: 11,
					color: "var(--dsw-alias-label-tertiary)",
					whiteSpace: "nowrap"
				} }, `点击卡片切换：${w.simToggle}`) : null) : null), react.createElement("button", {
					type: "button",
					className: "dsx-navbtn",
					"aria-label": "下一个",
					onClick: next
					...
					"aria-label": `${inst.w.name} ${inst.s === "2x4" ? "2×4" : "2×2"}`,
			...
			return react.createElement("div", { style: {
				...
				}, react.createElement("span", { className: "dsx-mhead" }, react.createElement("span", { className: "dsx-mname" }, GROUP_LABELS[groupOf(w)] ?? w.name), react.createElement("span", { className: "dsx-badge" }, String(instanceCount))), react.createElement("span", { className: "dsx-mdesc" }, w.desc), react.createElement("span", { className: "dsx-macts" }, react.createElement("span", { className: "dsx-btn" }, "查看详情"), react.createElement("span", { className: anyInstalled ? "dsx-btn dsx-btn-primary" : "dsx-btn" }, anyInstalled ? "已添加" : "添加")));
			})));
		}
		...
		function SettingsPanel({ controller }) {
			...
			return react.createElement("div", { style: {
				...
				title: "隐藏输入框下方文字条",
				desc: "隐藏输入框下方状态统计条的文字（保留原空间、不影响布局）；关闭时正常显示",
				children: react.createElement("label", { className: "dsx-switch-row" }, react.createElement("input", {
					type: "checkbox",
					className: "dsx-switch-input",
