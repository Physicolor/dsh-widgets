	...
	factory: (require) => {
		...
		*  today on the right, or calendar-quarter aligned) is user-configurable. */
		function heatmapRender(stats) {
			...
			return {
				title: "Token 用量",
				legend: todayVal > 0 || total > 0 ? `今日 ${fmtTokens(todayVal)}  ${fmtTokens(total)}` : void 0,
				chart: {
					kind: "heatmap",
					heatmap: grid
				}
		...
		*  custom text, show-title toggle, alignment, wrap. */
		...
		const WIDGETS = [
			...
			},
			{
				...
				group: "data",
				name: "用量热度图",
				desc: "最近 3 个月每日 Token 用量热度图（自记账）；可在预览选择窗口对齐方式",
				builtin: true,
		...
		*  8/16, then 8/14). Real-time usage keeps accumulating on today. */
		const SEED_DAY = "harness-widgets.heatmap.seeded.4";
		...
		function loadHeatmapBaseline() {
			try {
				...
				return {
					total: 0,
					date: dateKey(/* @__PURE__ */ new Date())
				};
			}
		}
		function saveHeatmapBaseline(n) {
			try {
				localStorage.setItem(HEATMAP_BASELINE, String(n));
				localStorage.setItem(HEATMAP_BASELINE_DATE, dateKey(/* @__PURE__ */ new Date()));
			} catch {}
		}
		const DEFAULTS = {
			panelPadding: 24,
		...
		*/
		function apply(ctx) {
			...
			ctx.slots.inject("conversation.composer.dock", () => ctx.slots.register({
				...
				const lastTotalRef = react.useRef(baselineInit.total);
				...
				react.useEffect(() => {
					...
					if ((localStorage.getItem(HEATMAP_BASELINE_DATE) ?? "") !== todayKey) {
			...
			ctx.slots.inject("shell.overlay", () => ctx.slots.register({
				...
				if (hasFocus) {
					if (active) {
						if (multi) {
							...
							const rowH = side + pad;
							const cx2 = (i) => (colIndexOf[i] + spanOf(i) / 2) * cellW;
							const cy2 = (i) => rowIndexOf[i] * rowH + side / 2;
							const fx = focusX;
							const fy = focusY;
				...
				if (anchor >= 0) {
					if (multi) {
						const cellW = side + pad;
						const rowH = side + pad;
						const cx = (i) => (colIndexOf[i] + spanOf(i) / 2) * cellW;
						const cy = (i) => rowIndexOf[i] * rowH + side / 2;
						const axc = cx(anchor);
						const ayc = cy(anchor);
						for (let i = 0; i < n; i++) {
					...
					} else for (let i = 0; i < n; i++) scaleArr[i] = stepScale(Math.abs(i - anchor));
				}
				const placeCards = (sc) => {
				...
				const rail = react.createElement("div", {
					...
					style: {
						...
						overflowX: "visible",
						boxSizing: "border-box",
						padding: `2px ${pad}px ${pad}px ${pad}px`,
						background: "transparent",
						pointerEvents: "auto"
					},
					onMouseLeave: () => {
					...
					onMouseMove: prefs.realTime ? (e) => moveRailFocus(e.clientX, e.clientY, e.currentTarget) : void 0,
					onScroll: (e) => {
				...
				const magnifyLayer = magnifying ? react.createElement("div", {
					...
					style: {
						position: "fixed",
						top: "calc(var(--dsx-rail-top,0px) - var(--dsx-rail-scroll,0px) + 2px)",
						right: "var(--dsh-sidebar-width, 0px)",
						width: `${railW}px`,
						pointerEvents: "none",
						zIndex: 25,
