window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*  system/tools/messages segmented bar + per-segment rows. Purely informational. */
		function contextWaterRender(stats) {
		...
		*  top-right brand-blue round → armed「确认」capsule (two taps to compact). */
		...
		function heatmapRender(stats) {
			const grid = stats.heatmapGrid;
			if (!grid || !grid.length) return null;
			const now = /* @__PURE__ */ new Date();
			const todayKey = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}-${String(now.getDate()).padStart(2, "0")}`;
			let todayVal = 0;
			for (const row of grid) for (const c of row) if (c.date === todayKey) {
				todayVal = c.value;
				break;
			}
			return {
				title: "Token 用量",
				headRight: todayVal > 0 ? `${Math.round(todayVal / 100) / 10}K · 今日` : void 0,
				chart: {
					kind: "heatmap",
					heatmap: grid
				}
		...
		*  custom text, show-title toggle, alignment, wrap. */
		...
		const WIDGETS = [
			...
			},
			{
				...
				group: "data",
				name: "用量热度图",
				desc: "最近 3 个月每日 Token 用量热度图（自记账）；可在预览中调整当前月落在左/中/右",
				builtin: true,
		...
		*/
		...
		function ChartBlock({ chart, side }) {
		...
		function CardBody({ out, side, onAction }) {
			const scale = side / BASE_SIDE$1;
			...
			const footStyle = vj || out.headRight || out.headAfter ? {
				flex: 1,
				minHeight: 0,
				display: "flex",
				flexDirection: "column",
		...
		function ConfigTab({ controller }) {
			...
			const selWidget = selected ? WIDGETS.find((x) => x.id === selected) : void 0;
			...
			const previewOut = () => {
				...
				if (selWidget.id === "heatmap") {
					const pos = selConfig.monthAlign === "right" ? "right" : selConfig.monthAlign === "left" ? "left" : "center";
					const grid = [];
					const anchor = pos === "right" ? 12 : pos === "left" ? 1 : 6;
					const now = /* @__PURE__ */ new Date();
					const startOfWeek = new Date(now.getFullYear(), now.getMonth(), now.getDate() - now.getDay());
					const base = new Date(startOfWeek);
					base.setDate(base.getDate() - anchor * 7);
					const day = (r, c) => {
			...
			return react.createElement("div", { style: {
				...
				fontWeight: 600,
				color: "var(--dsw-alias-label-primary)"
			} }, `${selWidget.name} · 预览`), out ? react.createElement("div", { style: {
				display: "flex",
				justifyContent: "center",
				padding: 8
				...
				out,
				side: 150
			})) : null, selWidget.configSchema && selWidget.configSchema.length > 0 ? react.createElement("div", { style: {
				display: "flex",
				flexDirection: "column",
				gap: 4
			} }, react.createElement("div", { style: {
		...
		*  left edge. Columns ahead of today render empty (value 0), shown faint. */
		function buildHeatmapGrid(m, position = "center") {
			const weeks = 13;
			const now = /* @__PURE__ */ new Date();
			const startOfWeek = new Date(now.getFullYear(), now.getMonth(), now.getDate() - now.getDay());
			const anchorCol = position === "right" ? 12 : position === "left" ? 1 : Math.floor(weeks / 2);
			const base = new Date(startOfWeek);
		...
		*/
		function apply(ctx) {
			...
			ctx.slots.inject("conversation.composer.dock", () => ctx.slots.register({
				...
				const heatmapRef = react.useRef(seedHeatmapIfNeeded());
				...
				react.useEffect(() => {
					...
					} else if (lastTotalRef.current === 0 && usage && inputTokens + outputTokens > 0) lastTotalRef.current = inputTokens + outputTokens;
					...
					setState({ stats: {
						...
						heatmapGrid: buildHeatmapGrid(heatmapRef.current, prefs.cardConfigs?.heatmap?.monthAlign || "center")
					} });
				}, [
					settled,
					projected,
					...
					runningCalls,
					now,
					prefs.cardConfigs?.heatmap?.monthAlign
				]);
				return null;
			}));
