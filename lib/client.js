	...
	factory: (require) => {
		...
		*  instead of throwing and taking the whole rail down with them. */
		...
		function usageBarsRender(stats) {
			...
			const u = data?.usage;
			const cycle = cycleFor(stats);
			const r = winPct$1(u, "rolling");
			const w = winPct$1(u, "weekly");
			const m = winPct$1(u, "monthly");
		...
		*  beyond what the magnitude needs). */
		...
		function ccWhoamiRender(stats) {
			...
			if (!user) return {
				title: t("widget.cc-whoami.name"),
				value: "—",
				legend: t("cc.notConfigured")
			};
		...
		function ccUsageRender(stats) {
			...
			const legend = req !== void 0 ? `${req} ${t("cc.requests")}${success !== void 0 ? ` · ${success.toFixed(0)}% ${t("cc.successRate")}` : ""}` : void 0;
			return {
				title: t("widget.cc-usage.name"),
				headAfter,
				legend,
				sub: cost !== "—" ? `${t("cc.spend")} ${cost}` : void 0
			};
		}
		...
		function ccCreditsRender(stats) {
			...
			const legend = [credits?.freeCredits !== void 0 && credits.freeCredits > 0 ? `${t("cc.free")} ${fmtCredit(credits.freeCredits)}` : "", credits?.purchasedCredits !== void 0 && credits.purchasedCredits > 0 ? `${t("cc.purchased")} ${fmtCredit(credits.purchasedCredits)}` : ""].filter(Boolean).join(" · ") || void 0;
			const fh = windows?.fiveHour;
			const wk = windows?.weekly;
			const fhPct = winPct(fh);
			const wkPct = winPct(wk);
			if (fhPct === null && wkPct === null) return {
				title: t("widget.cc-credits.name"),
				headAfter,
				legend,
				sub: void 0
			...
			const bars = [];
			if (fhPct !== null) bars.push({
				label: t("cc.fiveHour"),
				value: Math.round(fhPct),
				ratio: fhPct / 100,
				tone: windowTone(fhPct, fh?.exceeded)
			});
			if (wkPct !== null) bars.push({
				label: t("cc.weekly"),
				value: Math.round(wkPct),
				ratio: wkPct / 100,
				tone: windowTone(wkPct, wk?.exceeded)
			});
			return {
				title: t("widget.cc-credits.name"),
				headAfter,
				...
				sub: [fh && fhPct !== null ? `${t("cc.fiveHour")} ${fmtReset(fh.resetAt)}` : "", wk && wkPct !== null ? `${t("cc.weekly")} ${fmtReset(wk.resetAt)}` : ""].filter(Boolean).join(" · ") || void 0
			};
		}
		/** cc-windows — 5h / weekly quota as two donuts (OpenCode-style rings). */
		function ccWindowsRender(stats) {
			const windows = cc(stats)?.credits?.windowLimits;
			if (!windows) return {
				title: t("widget.cc-windows.name"),
				value: "—",
				legend: t("cc.notConfigured")
			};
			...
			const mk = (label, pct, exceeded) => ({
				label,
				value: Math.round(pct),
				ratio: pct / 100,
				tone: exceeded === true || pct >= 95 ? "danger" : pct >= 75 ? "warn" : "success"
			});
			const rings = [];
		...
		function ccSubscriptionRender(stats) {
			...
			const subText = endLabel ? `${t("cc.periodEnd")} ${endLabel}` : void 0;
			const cancel = sub?.cancelAtPeriodEnd === true ? " · " + t("cc.cancelAtEnd") : "";
			return {
				title: t("widget.cc-subscription.name"),
				headAfter,
				sub: subText ? subText + cancel : void 0
			};
		}
		//#endregion
		...
		*  Registered with the locale service at apply() time. */
		const WIDGET_LOCALES = {
			zh: {
				...
				"cc.notConfigured": "未配置 COMMANDCODE_API_KEY",
				"cc.account": "账户",
				"cc.tokens": "tokens",
				"cc.requests": "请求",
				"cc.successRate": "成功率",
				...
				"widget.cc-whoami.name": "Command Code 账户",
				"widget.cc-whoami.desc": "当前 Command Code 账户身份",
				...
				"widget.cc-usage.name": "Command Code 用量",
				"widget.cc-usage.desc": "请求数、成功率、Token 与消费摘要",
				"widget.sys-gpu.name": "GPU 状态",
				"widget.sys-gpu.desc": "本机 NVIDIA GPU 显存、利用率与温度",
				"widget.cc-credits.name": "Command Code 额度",
				"widget.cc-credits.desc": "Credits 余额与 5h/周额度窗口",
				"widget.sys-rings.name": "CPU·GPU 环图",
				"widget.sys-rings.desc": "CPU / GPU 利用率双环形图",
				"widget.cc-windows.name": "Command Code 窗口",
				"widget.cc-windows.desc": "5h / 周 额度窗口双环图",
				"widget.sys-board.name": "系统监控",
				"widget.sys-board.desc": "CPU/内存/GPU 显存与利用率全指标环形看板",
				"widget.cc-subscription.name": "Command Code 套餐",
				"widget.cc-subscription.desc": "当前套餐与账期结束时间",
			...
			en: {
				...
				"group.commandcode": "Command Code",
				"cc.notConfigured": "COMMANDCODE_API_KEY not configured",
				"cc.account": "Account",
				"cc.tokens": "tokens",
				...
				"widget.cc-whoami.name": "Command Code Account",
				"widget.cc-whoami.desc": "Current Command Code account identity",
				...
				"widget.cc-usage.name": "Command Code Usage",
				"widget.cc-usage.desc": "Requests, success rate, tokens and spend",
				"widget.sys-gpu.name": "GPU Status",
				"widget.sys-gpu.desc": "Local NVIDIA GPU VRAM, utilization and temperature",
				"widget.cc-credits.name": "Command Code Credits",
				"widget.cc-credits.desc": "Credit balance and 5h / weekly quota windows",
				"widget.sys-rings.name": "CPU · GPU Rings",
				"widget.sys-rings.desc": "CPU / GPU utilization twin rings",
				"widget.cc-windows.name": "Command Code Windows",
				"widget.cc-windows.desc": "5h / weekly quota windows as rings",
				"widget.sys-board.name": "System Monitor",
				"widget.sys-board.desc": "CPU / memory / GPU VRAM and utilization dashboard rings",
				"widget.cc-subscription.name": "Command Code Plan",
				"widget.cc-subscription.desc": "Current plan and billing period end",
		...
		function ChartBlock({ chart, side, width }) {
			...
			if (chart.kind === "rings" && chart.rings && chart.rings.length) {
				...
				const items = chart.rings.map((rg, i) => {
					...
					const labelRow = typeof rg.label === "string" && rg.label.length > 0 ? react.createElement("div", { style: {
						display: "flex",
						alignItems: "baseline",
						gap: 3,
						whiteSpace: "nowrap",
						...
						fontVariantNumeric: "tabular-nums",
						lineHeight: 1
					} }, `${Math.round(rg.value)}%`), react.createElement("span", { style: {
						fontSize: `${Math.round(9 * scale)}px`,
						color: "var(--dsw-alias-label-tertiary)",
						lineHeight: 1,
						overflow: "hidden",
			...
			ctx.slots.inject("conversation.composer.dock", () => ctx.slots.register({
				...
				react.useEffect(() => {
					const refresh = () => {
						...
						fetch("/api/commandcode-usage").then((r) => r.json()).then((data) => setState({ commandCode: data })).catch(() => {});
					};
