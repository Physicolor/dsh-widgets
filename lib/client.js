window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/** The rail geometry that fits a budget: preferred — fewer columns — narrower — collapse. */ function resolveRailLayout(prefs, budget) {
			const pad = prefs.panelPadding;
			const wantColumns = [
				1,
				2,
			...
			if (side >= RAIL_MIN_SIDE) return {
				side,
				columns: 1,
				railW: widthOf(1, side),
				constrained: false
			};
			return {
				side: wantSide,
		...
		*/
		...
		function RailWave(props) {
			...
			const overlayTransition = !active || animPhase === "grow" || animPhase === "shrink" ? "transform 0.15s var(--ds-ease-in-out)" : "none";
			const rail = react.createElement("div", {
				...
				style: {
					...
					...ANCHOR_FOLLOW ? { transition: "transform var(--ds-transition-duration-slow) var(--ds-ease-in-out)" } : { transition: "right var(--ds-transition-duration-slow) var(--ds-ease-in-out), transform var(--ds-transition-duration-slow) var(--ds-ease-in-out)" }
				},
				onMouseLeave: () => {
					armedRef.current = false;
					setFocusY(null);
			...
			const magnifyLayer = react.createElement("div", {
				...
				const slotStyle = {
					...
					willChange: magnifying ? "transform" : void 0,
