		...
		*  variants share exactly the same source the real collector uses. */
		...
		function ConfigTab({ controller }) {
			...
			return react.createElement("div", { style: {
				...
				paddingTop: 10,
				borderTop: "1px solid var(--dsw-alias-border-l2)"
			} }, sizesOf(selWidget).length > 1 ? react.createElement("div", { style: {
				display: "flex",
				flexDirection: "column",
				gap: 4
			} }, react.createElement("div", { style: {
				...
				fontWeight: 600,
				color: "var(--dsw-alias-label-tertiary)"
			} }, "卡片大小"), react.createElement("div", { style: {
				display: "flex",
				gap: 4
			} }, sizesOf(selWidget).map((s) => react.createElement("button", {
				key: s,
				type: "button",
				className: "dsx-btn" + (selSize === s ? " dsx-btn-primary" : ""),
				onClick: () => setPreviewSize(s),
				style: { minWidth: 56 }
			}, s === "2x4" ? "2×4" : "2×2")))) : null, selWidget.configSchema && selWidget.configSchema.length > 0 ? react.createElement("div", { style: {
				display: "flex",
				flexDirection: "column",
				gap: 4
			} }, react.createElement("div", { style: {
		...
		function MarketTab({ controller, usageData }) {
			...
			if (previewGroup !== null) {
				...
				return react.createElement("div", { style: {
					...
					fontWeight: 600,
					color: "var(--dsw-alias-label-primary)"
				} }, w ? `${w.name}${curSize === "2x4" ? " 2×4" : " 2×2"}` : ""), react.createElement("button", {
					type: "button",
					...
					justifyContent: "center",
					alignItems: "center"
				} }, out ? react.createElement("div", { style: {
		...
		function SettingsPanel({ controller }) {
			...
			return react.createElement("div", { style: {
				...
				title: "隐藏输入框下方文字条",
				desc: "隐藏输入框下方的状态统计条，减少界面信息密度，专注对话内容；组件栏可查看同类数据",
				children: react.createElement("label", { className: "dsx-switch-row" }, react.createElement("input", {
					type: "checkbox",
					className: "dsx-switch-input",
				...
				const focusedAdd = (() => {
					...
					if (multi) {
						...
						if (colIndexOf[n - 1] + spanOf(n - 1) < columns) {
							...
							return {
								top: rowTopAcc[lastRow],
								right: lc.right + lc.w + pad
							};
						}
						return {
							top: focusLayout.reduce((m, c) => Math.max(m, c.top + c.h), 2) + pad,
							right: 0
						};
					}
					return {
