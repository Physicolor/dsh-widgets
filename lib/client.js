window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*  Mirrors the shell preview logic that used to live in components.tsx. */
		function previewStats$2(config) {
		...
		*/
		function planQuota(input, now) {
			...
			const elapsedDays = (now.getTime() - start.getTime()) / DAY_MS$2;
			const totalDays = (end.getTime() - start.getTime()) / DAY_MS$2;
			if (totalDays <= elapsedDays) return null;
			const projectable = elapsedDays >= MIN_ELAPSED_DAYS;
			const daysLeft = Math.max(1, Math.ceil((end.getTime() - now.getTime()) / DAY_MS$2));
			...
			const dayFraction = Math.max(MIN_DAY_FRACTION, Math.min(1, (now.getTime() - midnight.getTime()) / DAY_MS$2));
			...
			const paceDays = Math.max(1, Math.round((midnight.getTime() - paceFromMidnight.getTime()) / DAY_MS$2) + 1);
		...
		*  Flat 12 · scale, i.e. 12px at the plugin's 150px default, as it always was. */
		function cardInnerPad(unit) {
			return Math.round(12 * (unit / BASE_SIDE$1));
		}
		/**
		...
		*  clear gap under the title+subtitle row instead of filling the whole card. */
		...
		function ChartBlock({ chart, side, width, pad }) {
		...
		*/
		...
		function CardBody({ out, unit, width, squircle, cornerPercent, pinBox, onAction, onCycle }) {
			...
			if (out.headAfter) {
				...
				}, out.headAfter.big) : null, haLines.length > 0 ? react.createElement("span", {
				...
				}, haLines.map((line, i) => react.createElement("span", {
					key: i,
					style: {
						fontSize: `${Math.round(10 * scale)}px`,
						color: "var(--dsw-alias-label-tertiary)",
				...
				}, line))) : out.headAfter.small != null ? react.createElement("span", { style: {
					fontSize: `${Math.round(10 * scale)}px`,
					color: "var(--dsw-alias-label-tertiary)",
					fontWeight: 500,
					fontVariantNumeric: "tabular-nums",
