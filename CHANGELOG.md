...
All notable changes to [`dsh-widgets`](https://github.com/Physicolor/dsh-widgets), newest first. A feature release is a minor bump, a fix-only release a patch bump; a version that was bumped but never published is not listed on its own — its changes are folded into the release that actually shipped them.

Where each kind of detail lives:

| Layer | What it answers | Location |
| --- | --- | --- |
| README | what the plugin is, how to install it, the current release at a glance | [`README.md`](README.md) · [`README.zh-CN.md`](README.zh-CN.md) |
| This file | every release, entry by entry, with the measurement behind each change | [`CHANGELOG.md`](CHANGELOG.md) · [`CHANGELOG.zh-CN.md`](CHANGELOG.zh-CN.md) |
| GitHub Releases | the same entry as release notes, anchored to the commit (tag) that shipped it | repository → Releases |
| `docs/` | the raw evidence: CDP probe scripts, screenshots, JSON receipts, per-incident fix records | [`docs/`](docs/) |
...
## v1.7.0 — every Command Code pool on one card, switched with a tap

> A feature release. The Command Code provider's **Account rotation** card puts several subscriptions behind one install (`COMMANDCODE_API_KEY`, `COMMANDCODE_API_KEY_2` … `_4`), but the widget family only ever read the first one — the rail silently reported one account's quota while the provider was rotating over two. Every Command Code card now carries the whole pool and a tap cycles `AllUser → Physicolor → Sparxie → AllUser`, the pool's total beside each account's own figures. Measured 2026-09-20 against the two live accounts on this machine: `AllUser` month 14.5% (Σ20.24 of Σ140 allowance), Physicolor 28.9%, Sparxie 0.0%; week 7.1% pooled vs 14.2% / 0.0% per account. Everything below is re-checked by `docs/probe-cc-pool.mjs` (68 assertions) against the real upstream.

### Added — the host reads the whole pool, not just the primary key

- **`/api/commandcode-usage` aggregates every configured member.** `COMMANDCODE_API_KEY` plus the `_2 … _4` spares that the provider's Account-rotation card writes are all resolved through the same credentials seam, and each one gets its own four official endpoints (`whoami` / `usage/summary` / `billing/credits` / `billing/subscriptions`), still fetched and timed out independently — one failing endpoint yields `null` for that slice, one failing key yields `data: null` for that member, and the rest still render.
- **Members are labelled with their own account name**, read from that key's `/alpha/whoami` (`name` → `userName`), which is what the card legend shows (`Physicolor`, `Sparxie` — not `Key 2`). A member whose `whoami` did not answer falls back to `Key N`, and a name that repeats gets the key's masked tail appended, so the cycle can never show two indistinguishable entries.
- **The payload stayed backward compatible.** The top-level slices are still the first member's own four slices, exactly the shape every consumer read before pools existed; the pool rides alongside as `keys[]`. A single-key install therefore reads byte-for-byte what it read before (verified: legs of the probe render the same cards from a payload with `keys` stripped).

### Added — tap a card to switch accounts

- **Whole-card tap cycles the pool views.** All eight Command Code cards, plus the 额度管理 projection card that reads the same account, carry a cycle descriptor: `AllUser → <account names> → AllUser`, with the chain in the hover tooltip. The selection persists per card instance in `cardConfigs.ccView` — deliberately a different field from the OpenCode pool's `poolView`, so the two families never share a view and a Command Code tap never fires the multikey pool's `prefer` call (verified: every card's `cycle.store === 'ccView'`).
- **`AllUser` is the pool's total, summed field by field.** A window's percent is `sum(used) / sum(cap)` — a mean of percents would report 50% for one exhausted pool beside an untouched one, when the pair is really half its allowance used (measured: 5h 0.48% → 0.5%, week 7.09% → 7.1% over the two accounts). Usage counts, tokens and spend add; `successRate` is recomputed from the summed counts, because a member with zero requests reports 0 upstream and must not drag the pool's rate down. The account card shows the generic `AllUser` label with the member names beneath it — `AllUser` is deliberately the same literal in both languages.
- **The legend line carries the current view** beside the card's role word (`账户 · AllUser`, `窗口 · Physicolor`, `5h 窗口 · Physicolor`), so which pool a figure belongs to is never ambiguous — including on the single-window cards, whose role word is the only thing that says which window they are.
- **One key configured → no switcher at all.** The cards render exactly as before (bare role word, account name in the body, no cycle), which is what every install that never configured a second account keeps seeing.
- **Verified** by `docs/probe-cc-pool.mjs`: it loads the real host bundle with a fake context, calls the real `/api/commandcode-usage` handler against the live accounts, compiles this repo's `cc-view.ts` and runs the real render factories over that payload in three states — AllUser, each account, and a `keys`-stripped single pool — asserting the labels, the sums (counts, spend, balance, both windows, the month), the legends, the persisted field, the stale-view fallback to AllUser and the unchanged single-pool cards. **ALL PASS, 68 assertions** (`docs/probe-cc-pool-result.json`).

### Changed — the monthly window is summed per member, each against its own plan

- **The pool's month is the sum of its members' months.** The plan → allowance table is keyed by plan id, so reading the pooled balance against one plan's allowance reported **0%** for a pool that was 14.5% used. `monthlyWindow()` now sums each member's own window, measured against **its own** plan (two GOAT accounts = 2 × $70), which is also what the roadmap had asked for as an upstream-independent reading; a member whose plan is unknown keeps its own balance fallback instead of poisoning the pool. The reset shown is the earliest member period end — the next renewal.
- **A stale view can never show another account's numbers under a label it no longer matches**: a persisted `ccView` that no longer resolves (account renamed, key removed) falls back to `AllUser`.
- **The README's monthly-window note was stale** — it still described the abandoned `used / (used + remaining)` conservation form and listed multi-key support as a roadmap item. Both now match the code, and the remaining limitation is stated for what it is: pool discovery follows the `…_API_KEY` / `_2 … _4` naming convention, because the credentials service exposes `resolve(ref)` and has no "list the refs" seam to enumerate them.
- **The single-key probe had been failing on that same stale formula** since the plan-allowance fix; `docs/probe-cc-render.cjs` now asserts the allowance-based figure (`28.9%`, not `34.8%`) and that a single-pool payload carries no switcher.

### Fixed — 额度管理 fills its layout instead of announcing 「数据不足」, and its head follows the 上下文水位 shape

- **The card has no 数据不足 state any more.** Whatever is missing is FILLED: no payload yet → `-%` beside the 账期 line when one is known, with `—` for the two figures; a period too young to project → the real consumed percent; an account that has never been used → zeros. The old branch rendered the message into the title row's right slot, which is also what squeezed the blue title down to 「额…」 — the title is no longer ellipsized in any state.
- **A young period is a flag, not a null.** `planQuota` now distinguishes MISSING INPUTS (still `null`) from "no pace to extrapolate yet" (`projectable: false`), so the 6-hour floor stops blanking a card whose consumed percent, tokens and budget are all real. Measured on the second pool member (subscription created that morning, elapsed 0.98 h): the card prints `0.0%` + `账期 10-20` + `今日用量 0` + `今日推荐 251M` where it used to print 「数据不足」 with no period and no figures.
- **An unused member's zeros are its own.** The daily log is machine-wide, so it is only credited to an account that actually spent in its period (`todayTokens = 0` when its own rate is not derivable); its BUDGET still answers — remaining credits ÷ days left at the POOL's realised local-tokens-per-credit rate — because the balance and the period are both real. The pace never borrows that rate: projecting the machine's usage onto an account that has served nothing would invent a number.
- **The stacked grey block is CENTRED on the figure's line box**, and the two token figures keep the card's FLOOR. Baseline-aligning the block's first line (the 上下文水位 treatment for a single grey line) put `账期 10-10` ~8px BELOW the figure's floor and left only ~12px to the figures row; top-aligning the body put that row right under the 账期 line with ~55px of empty tile beneath it. `headAfter.smallLines` now centres, and the new `bodyAnchor: 'bottom'` restores the every-other-card posture for 额度管理 (`marginTop: auto` on the foot) — measured live: figures bottom → card bottom = **14px** against the 13px inner pad, head bottom 132.5 vs figures top 185.2. New standard gap between the title row and the figure row: **4px** at side 150 (the app's 4/8/12/16 rhythm), up from the 2px a plain grey caption needed.
- **`headAfter.big` now honours `valueTone`/`valuePulse`** (`dsx-stats-card-value` + the breathe class) wherever a card renders its figure — the over-budget escalation survives the move out of the title row; asserted in the real React tree, not just the render object.
- **GPU 利用率 uses the same head**: blue title on top, `0%` on the next row with `53°C · 0.0 GB` to its right, and its elastic sparkline still filling the box exactly (`card 160 = slot 160`). Its "not enough history to draw a line yet" state shows the current utilization + facts instead of a waiting notice.
- **The two figures are the same story again, exactly.** The budget divided by the rounded-up day count while the projection used the exact remaining span, so a pace within ~3–4% of the budget could exceed it while the card still read 98% (measured live 2026-09-20: pace 258.9M vs budget 253.4M, projected 98.5%). Both now use the exact span, which makes `projected > 100% ⇔ pace > budget` an identity rather than an approximation.
- **Verified live** by `docs/probe-card-head.cjs` (45 assertions, Playwright + a minted browser-session cookie): the card is read out of the RESTING deck (the magnify overlay carries stale copies of the same cards), every pool view is reached by a REAL tap, and each one is asserted for the head shape, the 4px gap, the grey-to-the-right geometry, the block staying inside the figure's line box, the figures' floor position, the absent 数据不足 and the two figures. Measured 2026-09-20: AllUser `49% | AllUser / 账期 10-10 | 77.2M / 612M`, Physicolor `99% | Physicolor / 账期 10-10 | 81.2M / 264M`, Sparxie `0.0% | Sparxie / 账期 10-20 | 0 / 251M`; screenshots in `docs/probe-card-head-*.png`. `docs/probe-quota-manage.mjs` covers the math side (per-member rows + the new degradation rules), `docs/verify-quota-manage-render.cjs` the real React tree.
- **The SSR probe's stale preview premise was fixed on the way**: its own `previewStats` had drifted to a ~103% month, so "the preview default is calm" had been failing — HEAD's math projects 102.8% for it (measured by running HEAD's `quota-math` against the same inputs). The preview now lands at ~70%, leaving the over-budget state to the sim toggle it exists for.

### Fixed — 额度管理's head carries the 账期 line alone, bottom-aligned with the figure

- **The pool view name is gone from the head** (`AllUser` / the account name). Two stacked grey lines read as clutter, so the head prints the `账期 10-10` line only — dropped to the head row's FLOOR so its bottom edge lines up with the big figure's. That is a new contract field (`headAfter.smallAlign: 'bottom'`); the default stays baseline-aligned, which is what 上下文水位's grey line keeps. Tapping the card still cycles the pool and every figure moves with it — the view is simply no longer labelled.
- **Measured live** by `docs/probe-card-head.cjs` (53 assertions, **ALL PASS**): `68% | 账期 10-10 | 473M / 616M` (AllUser), `132% | 账期 10-10 | 473M / 257M` (Physicolor), `51% | 账期 10-20 | 473M / 1.0B` (Sparxie); the 账期 line's bottom edge lands on the figure's to the pixel (Δ=0px in all three views), no view name appears anywhere on the card, and the two token figures still sit 14px off the card floor. `docs/verify-quota-manage-render.cjs` asserts the same on the rendered React tree (`align-items:flex-end`, no stacked block).
- **The probe's state restore moved into a `finally`** (and its host re-read retries): an ECONNRESET mid-run had left the room persisted on the last view the walk tapped.

### Fixed — 额度管理 measures the Command Code plan, not the whole machine

- **The card's token side is folded to the `commandcode` route.** Its percent, 今日推荐 and credits all described ONE plan while 今日用量, the recent pace and the credit→token rate were read off the machine-wide day map — so a parallel OpenCode Go pool was charged to the Command Code allowance (measured 2026-09-20: the card printed 今日用量 **758M** where that route itself served **474M**; the machine-wide day was **783M**, of which 309M was `opencode-go`). `stats.commandCodeDaily` now carries the scoped map and the card reads nothing else.
- **`/api/widgets-usage-daily?provider=<route>`** asks dsh-usage-center to fold that provider alone. The mode must be `only`: its `all`/`merge` modes keep the whole window on purpose and would have handed a named route the machine-wide map back. Without the parameter the route answers exactly as before, so the heatmap cards are untouched.
- **The two maps are kept apart, and a stale host degrades instead of lying.** The client requires the scoped answer to ECHO `provider=commandcode`; a host that has not been restarted ignores the parameter, and the card then prints `—` for its token figures rather than silently restoring the mixed caliber.
- **`docs/verify-cc-scope.mjs` arbitrates it independently**: it decodes every raw session log itself (no plugin code), folds per-day tokens PER PROVIDER ROUTE, and asserts the card's own caliber against both host maps and the real account payload — `commandcode` 473.7M vs machine-wide 783.3M on 2026-09-20, with `opencode-go` 309.5M excluded. `docs/verify-usage-daily-route.mjs` covers the route's two calibers offline, `docs/probe-quota-manage.mjs` and `docs/verify-quota-manage-render.cjs` now read the scoped map too.

### Fixed — an upstream slice that did not answer can no longer rescale the pool

- **A holey pool DEGRADES; it never reports a hybrid caliber.** The host route answers a failed upstream call with `null` for that slice (one dead endpoint must not blank the family), and the client used to absorb that silently: a member whose `billing/subscriptions` call dropped fell back to the RETIRED conservation caliber (`used/(used+remaining)`) while the pool's calendar took the surviving member's period, so 额度管理 printed **20.1% + `账期 10-20` + `今日推荐 58.6M`** where the whole payload says **16.6% + `账期 10-10` + 645M** (the screenshot that opened this fix, 2026-09-20); a member whose `billing/credits` call dropped collapsed the pool onto the surviving member's window and printed a false **117%** over-budget alarm. `monthlyWindowOf` no longer reads a missing subscription slice as "an exotic plan", and `monthlyWindow` returns `null` the moment any member's month cannot be placed — a partial sum reads LOWER than the truth, which is the one failure a quota card gives the user no way to notice.
- **The pool's calendar is ONE member's pair**, not the earliest start beside the earliest end (the live pool is 09-10 → 10-10 beside 09-20 → 10-20, so two different members supplied the two halves and the card measured a period nobody ever had). It follows the earliest reset still AHEAD, so a member that already rolled does not anchor the month, and the `账期` line now prints the LOCAL day of that instant instead of the UTC fields of the string.
- **One retry before a slice is reported missing.** `fetchSlice` re-asks once when the failure was fast and retryable (network error / timeout / 5xx / 429) with a shorter budget, so the worst case stays near the original 8 s ceiling; a 4xx is an answer and is not retried.
- **The collector asks again when the payload is degraded.** `ccPayloadDegraded` (a dropped slice, or a period that already ended) schedules ONE re-ask 15 s later, so a transient upstream failure is a blip rather than the session's answer. Verified end-to-end by the new `docs/verify-quota-reask.cjs`, which serves a real frozen payload with one subscription slice removed and samples the live card: `额度管理 | -% | AllUser | 账期 10-20 | 今日用量 10.1M | 今日推荐 —` at 8.4 s → `75% | 账期 10-10 | 今日用量 480M | 今日推荐 649M` at 38.3 s, with the page asking its second time on its own.
- **The credit→token rate needs a log that COVERS the period.** A log beginning mid-period understates the numerator, and an understated rate does not merely soften the pace — it inflates the pace measured in credits, which is how a freshly loaded page (whose fallback log held only today, before dsh-usage-center's map landed) turned a calm **75%** month into a **247%** red alarm with `今日推荐 1.5M`. `logCoversSince` now gates the rate, the pace and the borrowed pool rate (the pool rate is summed per member, over each member's OWN period), so the card prints the consumed percent and `—` instead of a number it cannot price.
- **`docs/probe-cc-pool.mjs` grew a pool-integrity section** — both hole shapes make the pool month unknown, the calendar is one member's pair anchored on a live period, the `账期` label is the local day, and a rolled period is flagged for the re-ask: **88 assertions, ALL PASS** against the live accounts.

### Changed — the 套餐 card shows a tier badge, and its preview walks the plan ladder

- **The plan is a badge, not the raw id.** `individual-goat` used to be the card's big figure and ran off the tile as `individual-goa…`; the card now prints the tier the official plan map names it (`GOAT`, `PRO`, `MAX`, `ULTRA`, `PROVIDER`, `GO`, `TEAMS PRO`), resolved by LONGEST prefix after normalizing exactly like the provider resolves it (so `individual-pro-v1` beats `individual-pro`). An id outside the table is neither invented into a tier nor printed raw: the vendor prefix is stripped and the rest uppercased, so the badge stays one short word.
- **The grey line is the billing period**, matching the 额度管理 card's `账期 10-10` wording (the 套餐 role word is gone — the badge already says what the card is); the badge sits on the card's floor, and only a non-active status or a cancelling subscription adds a small note under it.
- **A preview click walks the LADDER.** `WidgetExample` gained `simSteps`: when present, a click on a market / 组件配置 preview advances to the next simulated state instead of flipping the single boolean, so the 套餐 card cycles GOAT → PRO → MAX → ULTRA → PROVIDER → GO → TEAMS PRO → GOAT. Both preview surfaces share one `nextSim()` helper, and peak-pricing / 额度管理 keep their boolean flip untouched.
- **Verified live** by `docs/probe-cc-plan-preview.cjs` (26 assertions): it opens the real market panel, selects the 套餐 preview dot, and asserts the title, the period line, the badge on the floor (34px vs the legend's 18px), the absence of the raw id — then clicks the card seven times and requires exactly `GOAT → PRO → MAX → ULTRA → PROVIDER → GO → TEAMS PRO → GOAT` with the period line unchanged. `docs/probe-cc-pool.mjs` (73 assertions) covers the same card from the LIVE payload; screenshots in `docs/probe-cc-plan-*.png`.

### Changed — 用量 and 额度 join the tag caption/floor layout, and the preview stage became a true square

- **用量 (cc-usage): the facts moved to a FIGURES row.** The token total stays the big figure under the title with `tokens` to its right, but the single grey line (`4821 请求 / 100% 成功率 / 消费 $26.5`, ~150px in a ~126px slot) was ALWAYS ellipsized before the spend — the three facts are now a figures row (`请求 4821 · 成功率 100% · 消费 $0.4676`) on the card's floor, with no role word (the figure carries its own unit).
- **额度 (cc-credits): rebuilt as the official site's limit rows.** `69.16 credits` on top, then THREE rows — `5 小时 6%` / `周 2%` / `月 1%` (the name on the left, the percent hard right) each over a **24-cell segmented bar** (used cells at the window's urgency tone, the remainder a pale wash of the same tone). The old `kind: 'bars'` twin columns (plus their dashed grid and 25px of labels) are gone, along with the role word and the reset line. A new `WidgetChart` kind `quotas` carries the rows.
- **The preview stage is the tile.** Both preview surfaces now render the card inside a pinned unit square (`pinBox`), so an over-tall card is clipped by its own `overflow: hidden` exactly as the rail seats it — measured before this: the credits card rendered **200×250** in the market and read as a non-square rounded rectangle; now every preview card measures exactly 200×200.
- **The sparkline no longer has its stroke cut off at the bottom.** A 0% sample (an idle GPU is the every-day case) mapped to y = 100 and the plot box's `overflow: hidden` sliced its lower half-stroke — measured on the live 利用率 card: polyline bottom **754.0 == svg bottom 754.0** with a 2px stroke, which is the「底部截断」the user reported. The polyline now lives inside `[PAD, 100 − PAD]` (PAD = 3 viewBox units) while the FILL still closes on the true floor; measured after: polyline bottom **751.9** inside a box ending at 754. The time-label row was verified to sit inside the card's padding (bottom 768 vs the 769 floor). **Because the report was intermittent, that is not taken on faith**: `docs/probe-gpu-line-stability.cjs` samples the live card every 1.5s for 45s (30 samples — the stroke stayed 2.1px inside, the labels 1px inside, and the card height never left its slot), and `docs/probe-gpu-line-values.cjs` intercepts the rail's own sysinfo route to serve a synthetic **0 → 100 → 0 ramp**, so one render carries every y the line can take: the plotted range came back exactly `[3, 97]` of the viewBox, i.e. 2.1px of clearance at BOTH ends against a 2px stroke — no value can clip it (screenshot `docs/probe-gpu-line-ramp.png`).
- **The rows were sized to the 2×2 budget rather than guessed.** The first cut of the credits redesign measured 100px of rows in the 81px the 2×2 card has left after title + balance and clipped its last bar against the floor; the shipped metrics (9px label line, 2px gap, 9px bar, 5px between rows ⇒ 73px) were computed from the budget and verified live.
- **The spend figure reads three significant digits** (`$10.1`, `$0.468`) instead of four decimals (`$0.4676`): on a 2×2 tile the fourth decimal is noise, and the figure shares its row with two other facts. Above $100 it drops to whole dollars, below a tenth of a cent it says `<$0.001` rather than rounding a real spend to `$0` (measured live: `$27.35` → `$27.4`, `$0.4676` → `$0.468`).
- **The dashed sparkline: a miss was drawn as a BREAK.** The real cause of the 「底部截断」 the user kept seeing (their report was stable, with the GPU completely idle) was not the stroke at all: the host stamps a `null` sample whenever its `nvidia-smi` query times out, and the line renderer broke the polyline there — measured live 2026-09-20: `history.gpu` carried **26 nulls against 0 in `history.cpu`**, and the 利用率 card rendered **2 polylines (15 + 3 points)**, i.e. a dashed line the user read as cut off. `plotSamples()` now carries the previous reading across a run of up to 4 misses (the poll window IS the averaging window, and the last reading is the best estimate for a utilization graph — Windows' own graph does the same), drops leading nulls instead of inventing them, and still breaks on a longer run (a device that really went away, which the card's own no-GPU state also reports in words). Measured after: **1 polyline, 20 points** on both the rail card and the market preview. Regression-covered from both ends: `docs/probe-gpu-line-values.cjs` feeds a ramp WITH null runs and requires one 20-point line, `docs/probe-gpu-line-stability.cjs` requires `segments === 1` in every live sample.
- **Verified live** by two new GUI probes: `docs/probe-cc-cards-preview.cjs` (23 assertions — the strict 200×200 square, the three facts with the mock values on the floor, the three quota rows with 72 segment cells whose filled count follows the percents: 6%/2%/1% of 24) and the extended `docs/probe-card-head.cjs` (47 assertions, now also asserting the sparkline stays inside its plot box and the time labels inside the card padding). `docs/probe-cc-pool.mjs` (79) and `docs/probe-cc-render.cjs` (24) cover the same two cards from the live payload; screenshots in `docs/probe-cc-cards-*.png`.
- **The probe learned to tell an upstream miss from a layout defect**: the host route fetches each member's four endpoints independently, so a member can come back with a slice missing under rate limiting (observed while running the suite). The period assertion now records that as the upstream state instead of blaming the card, and the `Key N` label fallback is accepted as naming the view by position.

### Changed — the magnification wave is one spring, written every frame

- **The enter is a continuous spring now, not a CSS tween.** The old shape was a 200ms `settle` transition followed by an untweened `follow`, and no parameter could make it hold a *moving* target: writing a new focus on every pointer frame **retargets** a CSS transition, so it restarts from its current value and never gets past the ease ramp's slow start (measured frame by frame on the real GPU: walking the pointer onto a card gave **0.02–1.1 scale/s** where a single jump gave **3.5** — the enter animated at whatever speed the mouse happened to be moved at). Freezing the target so the tween could finish fixed the curve but left the geometry stale, and the card **snapped** to the live position the moment the freeze ended — the reported "wrong position as soon as I move over the widgets".
- **The progress is an explicit factor** written every animation frame: `displayed[i] = 1 + (target[i] − 1) · p`, where `target` is the live pointer geometry (always current, never a reason to restart anything) and `p` runs `WAVE_SPRING` between rest (`0`) and engaged (`1`). Both are continuous, so their product is: enter, follow and leave become ONE uninterrupted motion on one curve, in both directions, and an interrupted enter reverses from the live `p` instead of jumping. The spring is the Apple-parameterised pair (`response 200ms`, `bounce 0.1`) shared with `dsh-usage-center`'s chart tween — ≈250ms to settle, ~0.15% overshoot (≈0.4px at the default tier). `prefers-reduced-motion` writes `p` in a single frame.
- **The phase machine is gone**: `animPhase` / `settle` / `follow` / `return`, `setPhaseNow` / `setPhaseAfter` / `phaseTimer`, `MORPH_MS` / `SETTLE_MS`. `leaveRail` is now just `armed = false; focus = null` — releasing is the same spring read backwards, so there is no second path to keep in sync and no "tween finished?" boundary to land on the wrong side of.
- **The enter's remaining cost was a compositing one, and the fix is a permanent hint.** The overlay's slots and the add button now carry `will-change: transform` **always**, so their 15 layers exist before the pointer arrives. Measured with the hint gated on the hover (the old shape): the natural enter opened with a **63ms** layer-promotion + raster frame; with it permanent the same enter ran **16ms with zero dropped frames** (`scripts/diag-hover-isolate.cjs`, P4 vs P5). The 64ms "raster prewarm" that used to flip the overlay visible while the rail was idle is REMOVED — it almost never coincided with a real hover, and the stall came back on every enter without the permanent hint.
- **Verified frame by frame** by `docs/verify-rail-morph.cjs` (`HEADFUL=1` for the real GPU, `REDUCED=1` for the reduced-motion path): it samples every frame of a stepped enter, a direct enter and a stepped leave and reports the per-frame speed envelope, so a retargeted curve or a snapped hand-over cannot pass unnoticed — a walked enter peaks at `maxFrameStep` **0.022** against **0.181** for the CSS version at the same geometry, and the reduced-motion enter arrives in one frame. The state machine and the scroll are regression-covered by `scripts/diag-rail-hover-release.cjs` (11 assertions) and `scripts/diag-rail-scroll-perf.cjs`.

### Added — the Settings nav cell wears the app's own glyph

- **The 组件 cell in Settings no longer shows the generic gear.** The settings shell picks a nav cell's glyph from the section ID and only knows its three built-in IDs (`models` / `agent-presets` / `plugins`); every other section —this one included —falls back to the settings gear, and the section contract carries no icon field (only `id` / `order` / `label`). The client half therefore marks OUR row itself: it finds the nav button whose label is our own registered section label and stamps `data-dsx-nav="widgets"` on it, and `widgets.module.css` hides the gear and paints the app icon's four-tile motif as a 16px `mask` in `currentColor`.
- **The mark is scoped and reversible.** Only that one row is touched —matched by our own label, never by position, so no other plugin's cell is styled —it is re-applied on a language switch (the label is localized), and it is removed with the owning effect, so closing the panel or unloading the plugin leaves no marked node behind. The observer only scans when a `_navList` is inserted, so the transcript's per-frame mutations never pay for it.
- **It had to be restored, not invented**: the CSS half and this mark were written together earlier, but only the CSS landed in a commit —the marking effect was lost with an uncommitted `index.ts` rewrite, which left a rule that matched nothing and put the gear back. (Same failure mode as the loading skeletons below; both halves now live in the tree and in the probes.)
- **Verified live** by `docs/verify-nav-glyph.cjs`: it drives the real settings panel and asserts that exactly ONE row carries the attribute and that the row's label is our registered section label, that the gear svg on that row computes to `display: none`, that its `::before` is a 16×16 inline-SVG mask, that no other nav row is marked, and that zero marked nodes survive the panel closing (screenshot `docs/verify-nav-glyph/nav-glyph.png`).

### Fixed — loading skeletons draw the silhouette of the card they stand in for

- **Every data-backed card had collapsed onto the same generic placeholder** (a value pill and two thin lines), so a loading rail said "something is loading here" but not WHICH cards were about to arrive — and the loading → loaded swap re-shaped each tile. `SkeletonBody` has supported the seven silhouettes all along (and `WidgetRenderOut.skeletonShape` / `skeletonCount` / `skeletonRows` are part of the public contract), but the shell-side `SKELETON_SHAPE` table and the wiring that fills it in were LOST with the same uncommitted rewrite as the nav glyph above: the loader only ever set `skeleton: true` plus a per-source row count, so `skeletonShape` defaulted to `'text'` for all 19 live-source widgets. The CSS-free half was there; the producer was not.
- **The table is restored, and it is per widget**: `usage-rings` / `cc-windows` draw three squares (rings), `sys-rings` two, the 2×4 `sys-board` four; `usage-bars` and `sys-gpu-line` draw ONE large rounded rectangle (a plot area); `cc-usage` a three-figure row on the floor, `quota-manage` a two-figure row, `cc-credits` three stacked quota bars; and the remaining big-figure cards (`usage-rolling` / `weekly` / `monthly`, `cc-whoami`, `cc-subscription`, `cc-window-*`, `sys-cpu`, `sys-gpu`) draw the value pill plus **one** body line — the honest placeholder for cards whose real body is a single grey line (a reset date, a billing period, a memory line), where two rows promised content that never arrives.
- **Verified against all 19 instances**, not just the ones installed on this rail: `docs/verify-skeleton-shapes.cjs` holds the four live routes open forever, injects every data-backed `widget@size`, measures each skeleton's blocks (count, geometry, row/column order, radius, overflow) and judges it against the expected silhouette — **19/19 PASS** — then restores the user's own rail state from a snapshot taken before the run (`docs/lib/widgets-state.cjs`); screenshots in `docs/verify-skeleton-shapes/`.

### Added — 连续曲率圆角 (continuous corner curvature)

- **Card corners are superellipses, not circular arcs**: `corner-shape: squircle` ramps the curvature in from the straight edges, which is what makes an iOS/Figma-style rounded rectangle read as soft rather than cut. It is native CSS, so the card's border and `box-shadow` follow the same outline (a `clip-path` squircle would slice both off), and an engine without `corner-shape` degrades to a plain round corner with no other change.
- **Two controls, in Settings → Components → 组件设置**: the switch (default **ON** — the setting exists so a user can compare) and the **corner gear**, the radius as a share of the card's SHORT side (`12 / 16 / 20 / 24 %`, default 16%). Percent, not px, because the reference keeps the corner-to-side ratio: a 150px card and its magnified 190px copy must not get the same corner. `12%` reproduces the old fixed 16px at side 150.
- **The gear reaches everything that draws a card**: the rail (resting deck and the magnified overlay), the market / 组件配置 previews, and the loading skeletons. That wiring is what had been missing — `Prefs` declared both fields, the settings rows wrote them and the previews read them, but the rail's own `CardBody` call passed neither, so the setting only ever changed the previews; `DEFAULTS` was missing the two fields as well (`tsc` reported it).
- **Verified live** by `docs/verify-corner-shape.cjs`: on a 15-card rail it asserts every card carries `dsx-squircle` with the switch ON and the measured radius equal to `shortSide × gear%` (26px at side 160 for the 16% gear), that flipping the switch OFF removes the class from all 15 and flipping it back restores all 15, and that selecting the 24% gear widens every radius to 38px — then restores the values it found.

### Fixed — the Command Code cards no longer freeze on their mount-time numbers

- **「额度管理」's 今日用量 / 今日推荐 were permanently `—`**: the card reads the day map folded to the `commandcode` route (`stats.commandCodeDaily`), and the client NEVER requested that URL (`/api/widgets-usage-daily?provider=commandcode`) — the contract, the host route and the host's fold were all in place; only the line that asks for it was missing, so `daily` stayed `undefined` and both figures degraded to the dash that means "no such number exists". Wired up; on the real payload the card now renders 今日用量 **363M** / 今日推荐 **726M** (head 35%).
- **The Command Code monthly ring had vanished, and the quota card's head read `-%`**: one DEGRADED reply (HTTP 200 with a `null` upstream slice) makes `monthlyWindow()` return `null` on purpose — a partial sum would read low — and the client OVERWROTE the last complete payload with it, then only re-asked on mount / turn-settle. `ccPayloadDegraded()` was exported for exactly that "ask once more, shortly" and was never called. One poll in three sampled exactly that way (`keys[].data.*` all `null`, labels fallen back to `Key N`). A degraded reply now never replaces a complete one and schedules one retry 5 s out, and a failed request keeps the previous numbers instead of blanking them (stale-while-error).
- **Idle numbers never moved at all**: these windows are ACCOUNT-wide — another client spending against the same pool moves them with no turn HERE to hang a refetch on — while the old collector pulled only on mount and when a turn settled. That is exactly why 用量中心 moved and the rail did not: usage-center runs its own 2 s poll. The rail now pulls the account payload every 60 s while it is open with a Command Code card installed, pauses while the tab is hidden, and refreshes the moment the tab returns.
- **The cost is measured, not assumed** (2026-09-23): one account tick is four upstream reads per pooled key — 8 reads, 1.06–1.6 s, 4.7 KB on this two-key pool, ≈480 reads/hour — and the `provider=commandcode` day map is a usage-center memo key nothing else asks for, so a cold call re-folds the session logs: **10.6 s and 21.2 s** against 34 ms warm. It therefore runs on the turn-settle and mount paths only, never in a poll loop. All host routes gained a 20 s TTL + single-flight memo, so several tabs, the degraded retry and the two scopes of one settling turn collapse into one upstream pass / one fold (measured cold 1063 ms → 0 ms cached, 6 requests folding 3 times).
...
### Added — the rail sizes itself from the product's own transcript measure

- **Budget-derived geometry.** The rail may only claim the margin the reading measure leaves over: `budget = official conversation column width − official transcript measure − 74px box inset`. Both numbers are read from the product's own published variables (`--dsh-conversation-column-width`, `--dsh-chat-content-width`), with a live `getBoundingClientRect` read preferred when it exists — the product writes those variables *after* the track transition settles, so a single debounced read can latch a mid-animation value (that is what once drove the budget to 0 and left the rail hidden until the window was resized).
- **A degradation ladder instead of a fixed claim.** Preferred layout → one column fewer → a narrowed single column (side ≥ 100px) → yield the field entirely (`--dsx-rail-w: 0`, nothing rendered, the user's preferences untouched, and the rail returns by itself when the window grows again).
- **The rail can no longer squeeze the reading column.** The inset rule lost its `!important`; because the claim is computed from the measure it can never exceed the leftover margin, so the only possible failure mode is collapsing — never compressing the transcript below the product's `clamp(680px, …, 920px)` floor.
- **The header capsule reports the same state.** With no room it renders `data-space="tight"` (dimmed, `aria-disabled`, `cursor: not-allowed`) instead of openable-but-empty.
- **Measured** (viewport → rail / transcript): 1578 → 372 / 748 px, 1400 → 198 / 748 px, 1280 → 154 / 748 px, 1120 → yielded / 664 px. Every one of those transcript widths is the product's own measure for that column; before this change the rail hard-claimed 372 px and drove the transcript to 554 px (1280) and 394 px (1120).

### Changed — the right panel now swallows the rail instead of the rail fading beside it

- **Layer order was the real problem, not opacity.** Probed with `elementFromPoint` over a fixed test block injected into each candidate host: inside `shell.overlay` the block paints *above* the official panel (z-20 vs the panel's z-10), inside the conversation subtree it paints *below* it. The rail's host moved from `shell.overlay` to `conversation.input.overlay` — the official list slot for "floating entries rendered inside the resident composer card" — so the panel sweeps across the rail and hides it segment by segment, exactly like a drawer being covered.
- **Pinning instead of animating.** While a panel exists (opening, open, or closing) the rail pins to the viewport's right edge via one variable, `--dsx-rail-right`; with no panel that resolves to the same value as the conversation-column anchor, so the switch is invisible. Closing retreats the panel and re-exposes the rail progressively. The variable also drives the magnify overlay and the settings drawer, so no rail-owned surface can stay parked over the panel.
- **Panel presence is read as `max(inline target, measured width)`.** On the first frame of the close animation the frame's inline grid target already reads `0px`; trusting it would snap the rail back under a panel that is still covering it.
- **One curve for track and occupant.** The transcript inset now uses the official `--ds-transition-duration-slow` / `--ds-ease-in-out` tokens instead of `0.2s ease`, matching the track's own animation — the layout package lists "track and occupant share one curve" as an invariant, and 0.2s/ease was visibly out of step with the 0.3s track. The composer-overlay seat (which is positioned against the scroll body's padding box, so it follows the rail's width) is on the same rule.
- **Invisible surfaces stop eating pointer events.** `[data-yielded]` (faded out because the panel owns the space) and `.dsx-stats-drawer[data-no-room]` (faded out with nothing covering it) now set `pointer-events: none`, so a see-through rail can no longer swallow hovers and clicks meant for the transcript or for the product's column-width drag band.

### Changed — native rail follow via CSS anchor positioning

- **The rail rides the product's own layout pass.** `[data-phase='active']` (the conversation host — measured to share the center column's box exactly; the hashed `_centerCol` class remains as a fallback, selected only when that seam is absent, so exactly one element ever owns the name) carries `anchor-name: --dsx-center`, and the rail's `right` is `anchor(--dsx-center right, var(--dsx-rightbar-w, var(--dsh-sidebar-width, 0px)))`. Anchor resolution happens in the same style→layout pass that animates the shell's `grid-template-columns` track. Measured per frame: `|center column right − rail right| = 0.00px` over a full open/close.
- **The JS follow path stopped writing a `:root` variable every frame** (a per-frame root-variable write invalidates style for the whole document); it is now only the fallback for browsers without anchor positioning, and it writes the settled value once, 200ms after the motion ends.
- **The ResizeObserver is re-bound lazily on every measure** (`observeMeasured()`), which is idempotent and self-heals when the shell re-creates its nodes. The old observer was constructed during `apply()` — before the Web UI mounts its frame — so every `querySelector` returned `null` and it observed **zero** elements (verified in-browser: 19 live `ResizeObserver`s on the page, none on `[class$='_rightbarCol']`). That is why the rail used to sit still through the sidebar's 300ms track animation and then jump into place.
- **Start and end with the panel.** The AppFrame's `transitionrun` for `grid-template-columns` plus the frame's inline grid target let the rail predict the final column width on the animation's first frame (the observer's first delivery trails it by ~100ms), so the rail's retreat and the panel's push run on the same beat instead of the rail arriving late.
- **Boot no longer plays the enter animation** when the preference says "open" but the session is not known yet: that first-paint slide had no valid containing block for the anchor (a transformed ancestor), so `anchor()` resolved to nothing and the rail painted at the left edge for ~2.6s before jumping. Restored layouts now appear in place; a user-initiated open still glides.

### Fixed — hovering the rail re-rendered the entire rail on every pointer frame

- **Measured before** (headed Edge, 1600×900): p50 frame 12.5ms, p95 41.8ms, max 129ms, 15 frames > 26ms, 598ms of JS across 2.6s of hovering — against a 4.2ms idle baseline. **After**: p50 4.2ms, p95 8.3ms, 1 frame > 26ms, 193ms of JS, i.e. back at the idle baseline.
- **Root cause**: the magnification wave owned its focus state in the same component that also built the resting deck, the widget list and the always-mounted add panel, so every pointer frame re-rendered every widget's `render()` output and the settings page inside the add panel.
- **Fix**: the wave is now its own `RailWave` component; the deck and the add panel are built by the parent, whose element identity stays stable while the pointer moves, so React bails out of those subtrees entirely (only the overlay's 7–15 slot divs reconcile per frame).
- **Fix**: magnified copies are sized with `transform: scale()` about a `top-right` origin instead of `width`/`height` — the same geometry, verified slot by slot (visual width == `baseW × scale`, right-edge delta 0), with the per-frame cost moved to the compositor.
- **Fix**: the resting deck fades through a `.dsx-wave-deck.dsx-wave-on` class (cards and the add button) instead of an inline per-slot opacity, so engaging the wave no longer re-renders the deck at all.

### Fixed — three symptoms reported against the live GUI (2026-09-17)

Full measurements, raw receipts and probe scripts: [`docs/verify-report3/FIX-2026-09-17.md`](docs/verify-report3/FIX-2026-09-17.md).

- **New conversation left the rail painted over the hero (5/5 reproductions).** The bridge used `useState` + a subscription effect, and the new conversation's `conversation.input.overlay` entry mounts in the same frame window in which the previous session's `conversation.composer.dock` unmounts — if the `hasSession: false` emit landed between "new entry rendered" and "effect flushed", the fresh instance never saw it and kept `hasSession: true` (rail painted, `opacity: 1`, hit-testing, while the module-level subscriber had already removed `body.dsx-stats-active` — two consumers contradicting each other). The bridge is now `React.useSyncExternalStore` (re-reads the snapshot on subscribe, so a missed emit cannot strand an instance), the snapshot is rebuilt per emit (stable reference), and `body.dsx-stats-no-session` sets `visibility: hidden` on the whole drawer so a session hand-off hides instantly (a user-initiated close still glides). Measured: before, 5/5 runs left the rail on the page (observed up to 6s, still intercepting pointer events); after, 5/5 non-interactive within ~300ms and unmounted within 1.5s.
- **The turn navigator vanished under the rail.** The product hides its own navigator with a container query on the transcript scroller (`@container (width <= 900px)`) — the very box the rail's yield narrows. This is a real, accepted cost of the space contract: with the rail open at ≤1600px the scroller drops below 900px and DSH hides its own navigator. Keeping both (a 2-column rail *and* an always-visible navigator) would require narrowing only the message content and not the scroll box — a structural change deliberately not made in this release.
- **Hovering the navigator showed the column-resize cursor and started a drag.** The product paints the invisible 40px column-width drag band at `z-index: 8` and the navigator's sticky slot at `7`, so wherever they overlap the band wins the hit test — this reproduces with the rail closed, because the product's own `CONTENT_EDGE_BUDGET` (176px per side) reserves that gutter for the band and not for the navigator. The fix corrects the order, not the geometry: `[data-conversation-scroll] :has(> nav[aria-label]) { z-index: 9 }` — addressed by the `nav` it contains rather than a hashed class, so a product rebuild cannot silently drop the rule. Verified at the overlapping geometry (1600px viewport, 1200px transcript): hover resolves to the navigator (`isNav: true`), a click no longer starts a width drag (`width-drag started=false`), and the rail's own sizes are unchanged (1440 → 198, 1600 → 372, 1760+ → 372).

### Fixed — small text no longer inherits the transcript's typography, and date labels no longer wrap

- **Typography.** The rail now lives inside the conversation subtree, so it inherited the transcript's 14px/24px; card rules that only declare a `font-size` (the 10–11px grey legends, the date labels) then got a 24px line box and blew up the card's text height. `.dsx-stats-rail` now pins the app-level typography (`16px` / `line-height: normal`, both measured on the app root); card rules that declare their own line-height still win.
- **`heatmap-bars` date labels.** "9.11" is 17px wide at 9px type, and the 7-column layout leaves each column ~14px, so the first/last date wrapped onto two lines, pushed the bars up and overflowed the 150px card (second line clipped by the card's `overflow: hidden`). The labeled columns, the line chart's corner labels and the heatmap's corner dates now carry an explicit `white-space: nowrap`; the labelled columns are the outermost ones, so the ~3px spill is symmetric and stays inside the card padding. Verified against the real DOM: 17×10px single line (was 14×15/14×18px two lines), seven label children with `scrollWidth == rect width == 17`, card height back to 150px.

### Fixed — `context-water` 2×2 no longer bursts its 150px slot

- The segments chart is the only chart whose body is three stacked legend rows, so it is what sizes the card. With the (now explicitly pinned) `normal` line-height the legend line box resolved to 16px — 20px per row — so the bar plus rows needed 85px where the slot leaves 79px, and the card grew to ~155–156px and overlapped the row below it.
- The legend rows now carry an explicit `1.2` line-height (font-stack independent) and the segmented bar's bottom gap drops from 10px to 6px (top gap stays 8px). Measured over CDP in Edge: card 150px (was 156px), chart 76.17px against 79px available.

### Fixed — the Components capsule kept its place after a bundle reload

- Placement in the header utility list is decided by `order` alone (registration sequence only breaks a tie), and `dsh-better-sidebar` registers its bottom-panel toggle at `order: 10`. Sharing 10 tied the two entries, so the capsule's position followed whichever fiber re-registered last — a tsdown/HMR reload or a market toggle pushed it past the toggle to the row's right end. The capsule now registers at `order: 5`: right of the official export control (0) and open-in-app (−10), left of that toggle, independent of load order.

### Website — from component gallery to design-system site

The showcase (`website/`, live at <https://physicolor.github.io/dsh-widgets/>) is no longer a gallery of cards. Full description: [`website/README.md`](website/README.md).

- **Design philosophy**, six principles, each bound to a real widget and a real geometry value from the source.
- **DSH Widget Design Grammar** — the real constants (`cardSide 150`, `panelPadding 24`, `magnify 1.2`, `scale = unit/150`, `innerPad round(12*scale)`, `radius 16`, type `13/20/10`, foot gap 6, corner `8*scale`, `stepScale peak/r3/t^1.6`) plus an interactive rail that runs the plugin's own magnification curve and right-anchored reflow; the sliders are the product's own settings.
- **Widget Anatomy** — a real card at a fitted unit, with padding, radius, type scale, title→content step and action inset annotated from measured DOM rectangles (the label set is reduced on narrow stages).
- **DSH Visual Audit** — 13 rules in two explicit layers: geometric measurement via `getBoundingClientRect` on the rendered cards, and declared design heuristics, scored as `round(100 * sum(w)/n)` over all 33 widgets, each measured at the size it ships in. The exception list reports the real findings (`context-water` +4px, `sys-gpu-line` +29px and `cc-credits` +33px past the magnified unit, the `cc-window-monthly` copy/implementation mismatch, the `sys-board` 2×4-only size contract). No claim of a beauty metric.
- **Component detail** — a per-widget dialog with both size variants, measured geometry, the rule results and the source entry.
- **Discoverability** — canonical, Open Graph, Twitter card and a generated OG image; JSON-LD (`WebSite` + `SoftwareApplication` + an `ItemList` of all 33 widgets); `robots.txt` + `sitemap.xml`; a title/description that names the project, DeepSeek Harness and the design-system framing. The gallery markup, `js/data.js` and the `ItemList` are now generated from the manifests by `website/gen-site.mjs` (`--check` wired into `verify`), so widget names/ids/categories/sizes/descriptions are present in the HTML source for crawlers and no-JS readers. The temporary `website/sync-data.mjs` (hardcoded path) is gone.
- **Page fixes found while auditing** — grid tracks can no longer be widened by content (a 2×4 preview used to blow out the 4th gallery column and made every 4th card wider than its row), the hero wordmark no longer out-weighs the widget array, and widget units are previewed at their natural size.
- **Verify**: 80/80 checks (static, SEO surface, crawlability, grammar math, anatomy measurement, audit, dialog, i18n, responsive, console/network) plus section screenshots.

### Known costs and assumptions (accepted)

- With the rail open at ≤1600px, the yield pushes the transcript scroller below the product's `900px` container query, so DSH hides its own turn navigator (see above).
- The rail's follow strategy prefers the official `data-phase` seam and falls back to the hashed `_centerCol` class; a future shell that renames both would need a new seam.
- `body.dsx-hide-statsline` still hides the whole composer-dock seat rather than the official `stats` entry; the rail does not yet suppress its transition while the official right bar is being dragged.

## v1.5.0

> Everything since v1.4.1 ships at once: the Command Code account widget family and its host aggregate route, DeepSeek Harness 0.1.5 compatibility and official right-bar adaptation, the new **Quota Manager** widget, the daily-usage freshness fix, and a unified text-level alert. The `###` sections below shipped in this release too (they were tracked as `Working tree (unreleased)` in the previous README).
